...
**Recommendation: Option B, and not yet.** It is two thirds of the benefit at zero numerical cost,
...
| **Bound** | **4.476e-36** worst case across 44.1–192 kHz, ≈ −707 dBFS, set by `ChorusEngine` at 192 kHz. **0 of 102,400 samples differ on real signal** in all three modules at every rate measured; the residual appears only on digital silence, where the dry term is `+0` and cannot absorb it. |
...
**Recommendation: approve, at lower priority than the pre-A7-2B numbers implied.** A7-2B absorbed
...
**Recommendation: accept it.** It is the lowest supported rate × smallest block × maximum density,
...
5. **A7-0** — whenever a named machine exists. Everything above was decided without it; the
   `PERFORMANCE_BUDGET.md` rows still cannot be filled honestly on a shared runner.
