...
juce::AudioProcessorValueTreeState::ParameterLayout createAnamorphLayout()
{
    ...
    layout.add (std::make_unique<AudioParameterChoice> (ParameterID { pid::channelMode, kVersion },
        "Channel Mode", StringArray { "Stereo", "Mono", "Left Only", "Right Only" }, 0));
    layout.add (std::make_unique<AudioParameterBool> (ParameterID { pid::swap, kVersion }, "Swap L/R", false));
    floatParam (pid::inputBalance, "Input Balance", { -1.0f, 1.0f, 0.001f }, 0.0f);
    layout.add (std::make_unique<AudioParameterBool> (ParameterID { pid::polarityL, kVersion }, "Polarity L", false));
    layout.add (std::make_unique<AudioParameterBool> (ParameterID { pid::polarityR, kVersion }, "Polarity R", false));

    // --- MS ---
    layout.add (std::make_unique<AudioParameterBool> (ParameterID { pid::msMode, kVersion }, "MS Mode", false));
    ...
    layout.add (std::make_unique<AudioParameterChoice> (ParameterID { pid::dimMode, kVersion },
        "Dimension Mode", StringArray { "Mode 1", "Mode 2", "Mode 3", "Mode 4" }, 1));
    floatParam (pid::dimAmount, "Dimension Amount", { 0.0f, 1.0f, 0.001f }, 0.5f, pct);
...
void ParamPointers::bind (juce::AudioProcessorValueTreeState& s)
{
    ...
    dimAmount     = s.getRawParameterValue (pid::dimAmount);
