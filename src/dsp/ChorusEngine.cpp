...
void ChorusEngine::processBlock (float* left, float* right, int numSamples) noexcept
{
    ...
    // fresh prepare() with Amount at its 0 default -- which is the state this path was written
    // for and measured in. It is NOT reached by turning Amount down: with a 0
    // target the update is `a -= wSmooth * a`, and under the block's
    // ScopedNoDenormals the DECREMENT underflows before `a` does, so the glide
    // stalls at ~FLT_MIN/wSmooth, which SCALES WITH THE SAMPLE RATE because
    // wSmooth is 1/(0.01*workingRate): 5.64e-36 at 48 kHz, 2.26e-35 at 192 kHz and the next decrement is exactly 0.
    // the `std::abs (currentWet) > 0.0f` test below therefore stays true and the full path runs instead. An earlier version of this comment claimed
