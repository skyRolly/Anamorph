...
|---|---|---|
| **Anamorph** | product name — `CMakeLists.txt:14, 232`, the plug-in's About screen, every artifact and installer | *`[OWNER/LEGAL DECISION]`* — whether the name is registered, pending or unregistered in any jurisdiction is **not recorded anywhere in this repository**. Use no ™ or ® symbol until that is settled. |
| **RollyTech** | company name — `CMakeLists.txt:227` (`COMPANY_NAME`), the `NOTICE` copyright line, the preset directory path, the bundle identifier `com.rollytech.anamorph`, and since 0.9.1 the four-character manufacturer code **`RTec`** (`CMakeLists.txt:229`, ADR-0023) that every RollyTech plug-in shares | *`[OWNER/LEGAL DECISION]`* — same: no registration status is recorded. |
