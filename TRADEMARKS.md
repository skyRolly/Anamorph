...
| **RollyTech** | company name — `CMakeLists.txt:151` (`COMPANY_NAME`), the `NOTICE` copyright line, the preset directory path, the bundle identifier `com.rollytech.anamorph` | *`[OWNER/LEGAL DECISION]`* — same: no registration status is recorded. |
