...
| JUCE's own usage reporting and splash screen are disabled | `CMakeLists.txt:507-508` (`JUCE_DISPLAY_SPLASH_SCREEN=0`, `JUCE_REPORT_APP_USAGE=0`) |
...
(`src/PluginEditor.h:1366`). It is an ordinary hyperlink: **Anamorph never opens it by itself.** If
...
| What | Where | When |
|---|---|---|
| **User presets** (`.anamorph`, XML) — sound-parameter values and a preset name you choose | your user application-data directory, under `RollyTech/Anamorph/Presets/` (Linux `~/.config/…`, macOS `~/Library/…` — JUCE's `userApplicationDataDirectory` is `~/Library`, without an `Application Support` segment — Windows `%APPDATA%\…`) — `src/PresetManager.cpp:58-59`, written at `:216-220` | the preset *file* only when you save a preset; the containing directory is also created the first time you open the **Load Preset** dialog (`src/PluginEditor.cpp:2308`, `dir.createDirectory()` in `showLoadPreset`) |
...
- the session's preset references — **three** of them: one for the preset you have selected and one
  ...
  (`PresetManager::encodeSelection`, `src/PresetManager.cpp:1371-1391`): a preset is recorded by its
