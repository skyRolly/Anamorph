...
| What | Where | When |
|---|---|---|
| **User presets** (`.anamorph`, XML) — sound-parameter values and a preset name you choose | your user application-data directory, under `RollyTech/Anamorph/Presets/` (Linux `~/.config/…`, macOS `~/Library/…` — JUCE's `userApplicationDataDirectory` is `~/Library`, without an `Application Support` segment — Windows `%APPDATA%\…`) — `src/PresetManager.cpp:56-57`, written at `:216-220` | the preset *file* only when you save a preset; the containing directory is also created the first time you open the **Load Preset** dialog (`src/PluginEditor.cpp:2035`, `dir.createDirectory()` in `showLoadPreset`) |
...
- the session's preset references — **three** of them: one for the preset you have selected and one
  ...
  (`PresetManager::encodeSelection`, `src/PresetManager.cpp:352-372`): a preset is recorded by its
