...
- **Defect mechanism (Verified from the pinned JUCE 9.0.0 source):** three distinct facts, only one of which is the crash.
  1. **The 残留.** `MenuWindow::windowIsStillValid()` exists to dismiss exactly this case — it fires when `componentAttachedTo != options.getTargetComponent()`. But both are `WeakReference<Component>` to `presetName`, an editor member: on the editor's death they null in the *same instant*, the comparison is false, and no dismissal happens.
  2. **The lost styling — NOT a use-after-free.** `PopupMenu` stores its look-and-feel as a `WeakReference<LookAndFeel>` (`juce_PopupMenu.h`), so `m.setLookAndFeel (&lnf)` nulled rather than dangled; `Component::getLookAndFeel()` then falls back to the default `LookAndFeel_V4`. Cosmetic, and it only trips the `~LookAndFeel` debug assertion about live weak references.
...
- **Prevention:** no headless GUI harness exists, so there is no automated regression test — the class of defect is instead prevented by construction (a parented menu has no independent lifetime to get wrong). Every other async callback in the editor was audited for the same shape; the file chooser was the only other one.
