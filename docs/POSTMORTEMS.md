...
- **Evidence [Partially Verified]:** CHANGELOG.md [0.8.6]. **[Verified]:** test `testMultibandEnableCrossfadeClickFree`; ADR-0004; src/dsp/AnamorphEngine.cpp:149-150, :513, :920-921 (`mbEnableBlend` crossfade).
...
- **Evidence [Verified]:** CHANGELOG.md [0.8.7]; commit 6a24b82; test `testSoloMultibandEnableClickFree`; src/dsp/AnamorphEngine.cpp:1975 (`soloMonitor.process`, every block); ADR-0004.
...
- **Symptom.** With host automation writing a sound parameter, a write landing between the apply and
  that read made the baseline describe the automated value: the preset read *unmodified* every time
  the automation returned to it, while reloading it would move the sound. Durable, and it travelled
  into A/B slots, undo snapshots and the session file.
- **Root cause.** The same two-read shape round 9 had removed from `saveUser`: the artifact (the
  file) is fixed, the live state is what moves, and a baseline read back from live state can be a
  baseline for something else.
- **Defect-formation mechanism.** The load path predates every D-2 round; §16 recorded its baseline
  as a self-correcting transient, a judgement made about a *restore* landing in the window (whose own
  adoption overwrites the baseline) and never revisited for an *automation* write, which nothing
  overwrites. Round 9's sibling audit found it and left it open deliberately: the symmetric fix
  (the save-side tree signature) made a just-loaded preset read *modified* at a measured 2 in 3000,
  and trading a rare false clean for a frequent false dirty was recorded as a product decision
  needing numbers.
- **Fix.** The number was found and the cost removed. A save's baseline describes live state the
  tree was captured *from*; a load's describes live state written *from* the tree and read back
  through the parameter's own store/report pair, which is one range mapping deeper and not
  idempotent in float for the four log-mapped frequency ranges. `soundSignatureAfterLoading` models
  that pass — the same arithmetic as the post-load live signature — and the load reconciles that
  prediction against one read-back per parameter at signature resolution (a bare prediction is
  bit-exact only if the compiler emits the arithmetic identically at both sites; the sanitizer build
  showed it does not). The baseline is what the parameters report, measured over 3000 round trips on
  both builds with no false dirty, and a foreign write during the load is detected by magnitude and
  left to read as dirty. The factory path uses the same formula over its override table.
- **Why this fix.** It makes the rule uniform across save and load: the clean baseline is derived
  from the artifact, and a mutation that lands during the operation leaves the preset dirty rather
  than being absorbed into its baseline.
- **Prevention.** State test 55 mutates a parameter at the seam between the apply and the baseline
  (both `load` and `loadFile`), walks every factory preset for exact cleanliness, and measures the
  load-side equality against the save-side one over 3000 random round trips so the reason the two
  flavours exist is a number. A read-back baseline fails 4 checks; the save-side formula fails 3.
