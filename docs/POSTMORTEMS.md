...
- **Why verification did not catch it:** three independent reasons, all structural rather than accidental.
  - **No automated surface reaches it.** `tests/state_tests.cpp:6-8` links the editor but never instantiates it; neither suite has a pointer or a display, and the defect needs a real modal pop-up plus a real click. pluginval (Level 4) drives a host we do not control and never opens a context menu.
