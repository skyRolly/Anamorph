...
`animToggle` 455–481 (`src/PluginEditor.cpp:2277-2302`). Taking each control's centre and
...
`src/PluginEditor.cpp:1620-1628`, which was the `rIn`/`rOut`/`rAct`/`rOn`/`rPos` easing block in
...
cited `src/PluginEditor.cpp:1577` at the merge base — the S11 generation pre-gate comment inside
...
predicted. `("PRIVACY.md", "src/PluginEditor.cpp:2101"): "createDirectory"` is therefore added to
...
two that do not are `titleButton` and `aboutLink`, and neither can produce the reported symptom:
  ...
  * **`aboutLink` has a live fallback but cannot be occluded by a menu.** It is the *only* child of
    `aboutBackdrop` (`src/PluginEditor.cpp:596`), so it is on screen only while the About overlay
...
label anchor `src/PluginEditor.cpp:590` pointed at `aboutLink.setFont` in `origin/main` and at
...
migration's own write. Re-aimed onto `uiScaleLabel` (`src/PluginEditor.cpp:619`) and onto the
...
`SpectrumImager::projectFromOrig` (src/gui/SpectrumImager.cpp:668) validated its pin arguments
...
**BENIGN — the four `C6001` PREfast reported.** `src/PluginProcessor.cpp:590`: `pid::viewParams`
...
Both consumers re-read a **live** `bandCount()`: `dragCrossoverTo` (src/gui/SpectrumImager.cpp:712)
...
to zero into the undo entry (`src/PluginProcessor.cpp:876-970`), so a bare `setParam` makes **no**
