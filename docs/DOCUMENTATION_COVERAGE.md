...
`animToggle` 455–481 (`src/PluginEditor.cpp:2301-2326`). Taking each control's centre and
...
`src/PluginEditor.cpp:1644-1652`, which was the `rIn`/`rOut`/`rAct`/`rOn`/`rPos` easing block in
...
cited `src/PluginEditor.cpp:1601` at the merge base — the S11 generation pre-gate comment inside
...
predicted. `("PRIVACY.md", "src/PluginEditor.cpp:2125"): "createDirectory"` is therefore added to
...
two that do not are `titleButton` and `aboutLink`, and neither can produce the reported symptom:
  ...
  * **`aboutLink` has a live fallback but cannot be occluded by a menu.** It is the *only* child of
    `aboutBackdrop` (`src/PluginEditor.cpp:601`), so it is on screen only while the About overlay
...
**Why.** Review finding *"relative navigation uses stale targets"* (`src/PluginProcessor.cpp:1488`).
...
**Why.** Review finding *"overlapping restores expose mixed sound"* (`src/PluginProcessor.cpp:1944`).
...
**Why.** Review finding *"pending edits become the clean baseline"* (`src/PluginProcessor.cpp:1759`).
...
label anchor `src/PluginEditor.cpp:595` pointed at `aboutLink.setFont` in `origin/main` and at
...
migration's own write. Re-aimed onto `uiScaleLabel` (`src/PluginEditor.cpp:624`) and onto the
...
**BENIGN — the four `C6001` PREfast reported.** `src/PluginProcessor.cpp:638`: `pid::viewParams`
...
anchored it, src/gui/SpectrumImager.cpp:1319 today: `dropX`
...
in that cell are now written in full (`src/InternalState.h:72; src/gui/SpectrumImager.cpp:1632`), and
...
"line 512 as PREfast anchored it, src/gui/SpectrumImager.cpp:1319 today", which keeps the fact and
...
to zero into the undo entry (`src/PluginProcessor.cpp:924-1054`), so a bare `setParam` makes **no**
...
`setBands` returns `stored && bandCount() == want` (`src/gui/SpectrumImager.cpp:882`) and
