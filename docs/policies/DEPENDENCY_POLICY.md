...
|---|---|---|---|
| **JUCE** | **9.0.2**, pinned by **immutable commit SHA** `72782788ce18c2d4d760b28e0921d6ffc6431102` | CMake `FetchContent` (`GIT_SHALLOW`), overridable via `-DANAMORPH_JUCE_PATH` | CMakeLists.txt:70-72, 81-89 (`9.0.2`) |
...
- **Clang major 22 → 23 — EVALUATED AND NOT TAKEN; the pin stays 22.** Recorded in **ADR-0033**
  ...
  this log exists to make findable. **The blocker, measured 2026-08-30:** every apt.llvm.org `-23`
  suite (noble, resolute, bookworm, trixie) carried
  the same commit `55feb0a3b6b7` (noble's package is
  `1:23.1.0~++20260818083557+55feb0a3b6b7`; the others differ only in build timestamp), built
  2026-08-18 — **49 commits before**
  `llvmorg-23.1.0` (`ea7d852a`, 2026-08-25), and **no Ubuntu series publishes `clang-23` at all**
  (Launchpad: 22 reaches `1:22.1.6-1ubuntu2` in stonking, 23 is unpublished). A release existing is
  not the same as a release being installable, and rule 1's *upstream stable* means the compiler,
  ...
  attempt starts from: 32/32 twin-dump scenarios bit-identical clang-22 vs clang-23, a
  `diff`-identical first-party warning census, 226+920 checks green on Release and 224+920 under
  ASan+UBSan+`vptr`, and an unmoved ABI floor. **The condition that lifts the block is single and
  checkable:** an apt.llvm.org `-23` suite rebuilt from `ea7d852a` (or a later 23.x *release* tag),
  at which point the identity line is added and the pin moves. No `src/`, `CMakeLists.txt` or
  packaging change; no `CHANGELOG.md` entry (rule 3 — not user-visible: the compiler did not move).
