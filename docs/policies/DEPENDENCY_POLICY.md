...
| **Clang** (the Linux **release** build `linux`, plus `merge-check`, the sanitizer, realtime and fuzz jobs) | **major pinned — 23**, upstream stable (ADR-0028 sets the rule; **ADR-0033** sets the current value — LLVM 23.1.0, released 2026-08-25, is the current stable line) | `ANAMORPH_CLANG_VERSION`, the single authority, consumed by every consuming job's install, their ccache lineages and `--clang-major`. Installed from **apt.llvm.org** by `scripts/setup-llvm-apt.sh`, which verifies the signing key by fingerprint and installs `clang-<n>`, `lld-<n>` and `libclang-rt-<n>-dev` together so the LTO link is version-matched by construction. **Since ADR-0030 this toolchain SHIPS the Linux artifact**, so a bump is no longer exempt from rules 2–3 | .github/workflows/build.yml:112-114 |
| **GCC** (`linux-lto-tests` only — the **compatibility** compiler; **ships nothing**) | **major pinned — 16**, patch deliberately unpinned: the floating `gcc:16` tag resolves to the newest stable 16.x | `ANAMORPH_GCC_VERSION`, consumed by the ccache lineage, the major assertion and `--gcc-major`. Supplied as the official upstream toolchain image because **no package source ships a released GCC 16** for a runner-available Ubuntu — noble stops at `g++-14`, and both `ubuntu-toolchain-r/test` and Ubuntu 26.04 carry only pre-16.1 trunk snapshots. A checker wants patch updates automatically; a shipping toolchain must not, which is why this is the one weak pin here | .github/workflows/build.yml:115 |
| Linux system libs | distro packages | `scripts/setup-linux.sh`, in two profiles (ALSA, JACK, X11, FreeType, GTK/WebKit, mesa, **EGL — required by JUCE 9's Linux GL context path**, plus `python3`; `full` adds the host toolchain, curl/unzip, xvfb and lld) | setup-linux.sh |
...
- **Clang major 22 → 23** — recorded in **ADR-0033** (`Accepted` 2026-08-30), firing ADR-0028's own
  revisit trigger: LLVM **23.1.0** was released 2026-08-25, so the rule ADR-0028 set (*upstream
  stable*, by major) selects 23. **Scope, and it is the one that changed:** since ADR-0030 this
  toolchain **ships the Linux artifact**, so this is the first bump to which rules 2–3 apply, and
  they were discharged rather than argued away. **Rule 2:** the committed harness `tests/dsp_dump.cpp`
  built from one source against one JUCE checkout with otherwise identical flags, differing only in
  the compiler — **32/32 scenarios bit-identical**, hashes over every output byte *and* reported
  latencies, with both binaries' `--self-check` passing so the instrument is discriminating; **226
  DSP + 920 state checks** green on the Clang 23 Release build and **224 + 920** under its
  ASan+UBSan+`vptr` build with `libclang-rt-23-dev` (224 because Test 38's malloc half is by design
  absent under ASan). The **Level-5 audition remains outstanding** — the twin dump leaves it nothing
  to discriminate in the engine, but it covers the editor path the dump does not reach. **Rule 3:**
  latency reporting is inside the identical twin-dump comparison, and session reload is the
  920-check state suite on both builds; the Linux ABI floor is unmoved at `CXXABI_1.3.9` /
  `GLIBC_2.38` / `GLIBCXX_3.4.31`. Detector coverage held: the first-party warning census came back
  `diff`-identical (9 entries / 17 sites, only `# clang-major:` moved), the
  `-Werror=function-effects` canary is live in both directions on 23, and `libclang_rt.rtsan` is
  present in the 23 install. **The trigger check itself was corrected**: `CURRENT_LLVM_STABLE` still
  read 22 five days after 23.1.0 shipped, so §Update mechanisms above now names upstream's release
  page and keeps that constant as corroboration. Reproducibility is weaker than at 22 and said so:
  `release/23.x` is an **open** branch, so the suite is not frozen. Synced: this table, this log,
  §Update mechanisms, `ARCHITECTURE_REVIEW_GATE.md` (one clause ADR-0030 had falsified), `CI_CD.md`,
  `BUILD.md`, `TESTING.md`, `ADR_INDEX.md`, ADR-0028/0029/0030 cross-links, `HANDOVER.md`. No `src/`,
  `CMakeLists.txt` or packaging change; no `CHANGELOG.md` entry (rule 3 — not user-visible: the
  engine bytes and the ABI floor are unchanged).
