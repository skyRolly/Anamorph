...
| GUI → Audio (forced duck / A/B switch / A/B forget) | `std::atomic<int> duckRequest` | One request word, relaxed: the duck bit, the A/B switch bit with its two slot indices, the forget bit. The message thread writes it with `fetch_or` / CAS so no request clobbers another; the audio thread (`setParameters`) and the prepare path (`primeParameters`) take it whole with one `exchange`. The per-slot Level-Match records it drives are engine-owned, audio-thread / prepare-path state; nothing engine-side is read back on the message thread (ADR-0007, Amendment of 2026-09-25, A/B provenance). |
