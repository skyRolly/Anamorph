...
| **1b** | Dynamic analysis | ASan + UBSan over both suites, then valgrind memcheck over both suites from an unsanitized build; **RealtimeSanitizer over the DSP suite** in its own lane (ADR-0029 — the driver forbids combining it with the others), behind a liveness canary, enforcing `REALTIME_AUDIO_POLICY` on the annotated audio entry point; `MALLOC_PERTURB_=1` on the per-push Linux self-tests (glibc fills fresh heap with `0xFE`, freed heap with `0x01` — the value is complemented for allocations, so it is **not** the fill byte) | `sanitizers` and `realtime` jobs in `.github/workflows/build.yml` |
