...
| **2** | Unit / behaviour | Deterministic DSP assertions + state-restoration robustness | `tests/dsp_tests.cpp` (28 DSP tests + 1 A/B state-restoration clamp guard) |
...
- **Level 2/3 self-tests must pass** (the headless gate, `scripts/run-tests.sh`): the 28 DSP
