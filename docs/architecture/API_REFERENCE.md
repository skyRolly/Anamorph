...
| `beforeStateCapture` | `std::function<void()>` | Test seam, empty in production (one null check, non-audio path). Fired by `saveUser` immediately before the one state capture the file and its baseline are both derived from — the instant a mutation would have to land to split them. State test 52 uses it. |
