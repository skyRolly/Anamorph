...
| `requestAbSwitch` | `void (int fromSlot, int toSlot) noexcept` | The A/B switch (message thread, lock-free; call before applying the destination slot's parameters): requests the masking duck and names the slot being left and the slot to restore. The engine records the slot being left — the published Level-Match value, whether it is **measured** (`LoudnessMatch::isResultMeasured`: the measure's confirmed answer, not merely current), and the state and sample rate it was measured for — and at the duck's silent bottom restores the destination's record: its value always, measured only if it was measured, at this rate, for the same measurement inputs (`measurementInputsDiffer`), otherwise not current. The bottom also re-arms the analysis when the two slots differ in anything the measurement reads (ADR-0007, Amendment of 2026-09-24, F13(2)). A pending switch keeps its source; a second switch while a restore is armed records nothing (ADR-0007, Amendment of 2026-09-25, A/B provenance). |
