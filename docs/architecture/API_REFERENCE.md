...
| `injectMatchGainDb` | `void (float) noexcept` | A/B per-slot Level-Match restore (atomic). Consumed at the switch's silent bottom, which also re-arms the analysis when the two slots differ in anything the measurement reads (ADR-0007, Amendment of 2026-09-24, F13(2)). |
| `requestDuck` | `void () noexcept` | Force a masking duck around a bulk param swap (atomic). |

Evidence [Verified]: src/dsp/AnamorphEngine.h:46-176.
