...
| `applyAutoGain` | `void ()` | Locks measured Level-Match gain into Output Gain. Does nothing while the published gain is NaN (ADR-0007, note 2026-09-24). |
...
| Member | Signature | Responsibility |
|---|---|---|
| `prepare` | `void (double sampleRate, int maxBlockSize)` | Allocates all buffers/oversamplers; resets; **then snaps every smoothed value onto the current snapshot's targets** — the engine's own via `snapSmoothers()`, and each module's via its `snapToTargets()`, which the modules' own `prepare()` cannot do because it runs before the snapshot is pushed in (ER-DSP-09). A restored session therefore opens IN its sound instead of gliding into it. The Level-Match published result survives a re-prepare at the **same** sample rate whose snapshot (`primeParameters`) changes nothing the measurement reads, while that result is **current** — the measure has caught up with the last change to anything it reads, however it arrived (a live edit, a duck bottom, a forced swap), and `reset()` will not adopt such a change from an in-flight duck (ADR-0007, Amendment of 2026-09-25; Test 69, State test 133); an A/B slot's remembered value restored at a switch is current only if it was **measured** when the slot was left, at this rate, for the measurement inputs it is restored into (ADR-0007, Amendment of 2026-09-25, A/B provenance; Test 70, State test 134) — the analysis is re-armed, the result kept, and with Level Match on the applied Level-Match gain starts on that result (not at unity, so audio resumed below the silence→audio detector plays matched from its first block; Test 68, State test 132); any other re-prepare flushes it and the applied gain starts at unity (ADR-0007, Amendment of 2026-09-24, F13(2)). (Allocation happens here, never in `process`.) |
| `reset` | `void (ResetScope resetScope = ResetScope::everything)` | Lands an in-flight switch duck on its target FIRST — a forced swap (A/B, preset load, undo, redo) as its silent bottom would, smoothers snapped; an ordinary duck's live controls keep gliding. For Level Match it is not the bottom's exact twin: the reset re-arms the analysis whatever the swap, a pending A/B restore (or engine-API injection) is adopted on the next `process()` block, and the applied gain lands at the next silence→audio edge rather than through a Case-A landing — then clears every audio tail (module delay lines and filter banks, the oversamplers, the four delay rings, the Level-Match analysis), settles the Bypass / Multiband Enable / oversampling crossfades and re-latches the oversampling engagement. `everything` (called only from `prepare()`) also clears the whole Level-Match matcher and the meters — except that when `prepare()` keeps the Level-Match result (same rate, same measurement inputs, a current result) the matcher is re-armed instead. `audioTailsOnly` (the host reset, `AnamorphAudioProcessor::reset()`) keeps the published Level-Match gain and the meter latches, clears the live meter display, and re-seeds the Chorus / Dimension-D wet and depth. The node resets land the glides they snap (Haas delay, crossovers, band widths, Band Solo split); no other glide a live edit left in flight is snapped — the engine's smoothers, the Haas / Velvet amount, Velvet density and Mono Maker cutoff keep gliding — so a host reset equals a clean start (Level Match aside) only when none of those is moving. |
...
| `requestAbSwitch` | `void (int fromSlot, int toSlot) noexcept` | The A/B switch (message thread, lock-free; call before applying the destination slot's parameters): requests the masking duck and names the slot being left and the slot to restore. The engine records the slot being left — the published Level-Match value, whether it is **measured** (`LoudnessMatch::isResultMeasured`: the measure's confirmed answer, not merely current), the state and sample rate it was measured for, and, when it is not measured, the measure's post-change evidence (`LoudnessMatch::Evidence`) — and at the duck's silent bottom restores the destination's record: at this rate, for the same measurement inputs (`measurementInputsDiffer`), a measured record as its value, measured, and one not measured as its post-change evidence's mean, measured, when that evidence is a measurement (share ≥ 0.5) — otherwise, and for any other rate or inputs, its value, not current (ADR-0007, A/B provenance, revision of 2026-09-26). The bottom also re-arms the analysis when the two slots differ in anything the measurement reads (ADR-0007, Amendment of 2026-09-24, F13(2)). A pending switch keeps its source; a second switch while a restore is armed records nothing (ADR-0007, Amendment of 2026-09-25, A/B provenance). |
| `forgetAbMatchMemory` | `void () noexcept` | A session restore replaced both slots: the next block resets every record to 0 dB, not measured, and drops a switch still pending (ER-STATE-20). |
| `injectMatchGainDb` | `void (float) noexcept` | Engine API (atomic): restore a Level-Match value the CALLER asserts is a measurement of the state it is adopted with — consumed at the next forced bottom (with the same re-arm rule) or, with no duck, the next block, and adopted as measured. The processor's A/B switch does not use it (`requestAbSwitch`). |
| `requestDuck` | `void () noexcept` | Force a masking duck around a bulk param swap (atomic; sets one bit of the request word, so it never cancels a pending A/B switch). |

Evidence [Verified]: src/dsp/AnamorphEngine.h:46-198.
...
Evidence [Verified]: src/PluginParameters.h:195-203, :74-87; .cpp:53-300.
...
| `editIsNewerThan` | `static bool (juce::uint32 editGen, juce::uint32 generation)` | The per-field precedence predicate of ADR-0036 §9, in one place: was this field edited at or after the restore being resolved against arrived? Used by `adoptResolved`'s write loop and by `resolvedWithEdits`, so the adoption on the message thread and a save on a host thread cannot answer it differently (§21). Wrap-around safe. |
| `editGenerations` | `EditGenerations () const` | A copy of the per-field edit generations, for publication inside `ProgramSnapshot` alongside the tree they describe (§21). **Message thread only**; a host thread only ever sees the copy. |
| `resolvedWithEdits` | `static juce::ValueTree (const juce::ValueTree& restored, const juce::ValueTree& live, const EditGenerations&, juce::uint32 generation)` | The Settings an adoption will produce, as a value: `restored` everywhere except a field edited at or after `generation`, which keeps `live`'s. What `adoptResolved` computes in place, for a reader that owns neither tree — a host-thread save inside the pending window (§21). Pure; safe on any thread. |
...
| `soundSignatureAfterLoading` | `static juce::String (const APVTS&, const juce::ValueTree& savedSound)` | The signature `soundSignatureFor` will return once `savedSound` has been loaded — the clean baseline `load`/`loadFile` set, computed from the values being written and never read back (ADR-0036 §18, closing KI-029). Differs from `soundSignatureForSavedTree` by modelling the parameter's own store/report pass (`normalisedAsRendered` applied twice), one range mapping deeper than a save's capture and not idempotent in float for the log-mapped frequency ranges. **Exact, with no tolerance** (§19): measured equal to the post-load live signature over 20 000 values and 3 000 full save/XML/load round trips in both the Release and the ThreadSanitizer build. Round 10 briefly reconciled it against a live read-back within 1e-6; that window absorbed real automation writes into the baseline and is gone. Its stated FMA-contraction premise could not have applied in the build where the equality failed (ADR-0031 compiles it `-ffp-contract=off`, 0 FMA emitted); the equality is asserted because it *is* the product invariant, not because every toolchain has been measured. |
