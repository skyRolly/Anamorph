...
- Source: src/dsp/AnamorphEngine.cpp:1024-1952 (`process`)
...
| Stage | Source |
|---|---|
| 0 · Input level tap | src/dsp/AnamorphEngine.cpp:1205 (`levels.input.process`) |
| 0b · True-bypass dry capture | src/dsp/AnamorphEngine.cpp:1207-1278 (`bypassDelayBuffer`) |
| 1 · Input conditioning | src/dsp/AnamorphEngine.cpp:1281 (`applyInputConditioning`) |
| 1b · M/S Solo | src/dsp/AnamorphEngine.cpp:1286-1289 |
| — conditioned-input capture | src/dsp/AnamorphEngine.cpp:1297-1298 (`dryScratch`) |
| 2a · Oversampled nonlinear region | src/dsp/AnamorphEngine.cpp:1300-1457 |
| 2b · Linear algorithm at base rate | src/dsp/AnamorphEngine.cpp:1459-1461 |
| 2c · Global Width (MS-domain) | src/dsp/AnamorphEngine.cpp:1463-1477 (`applyWidth`) |
| 2d · Multiband Width | src/dsp/AnamorphEngine.cpp:1479-1598 (`multiband.processBlock`) |
| 3 · Dry/Wet Mix | src/dsp/AnamorphEngine.cpp:1600-1708 |
| 4 · Mono Maker (post-Mix) | src/dsp/AnamorphEngine.cpp:1715 (`monoMaker.process`) |
| 5 · Output stage | src/dsp/AnamorphEngine.cpp:1717-1843 |
| 6 · Band Solo monitor | src/dsp/AnamorphEngine.cpp:1861 (`soloMonitor.process`) |
| 6b · NaN/Inf self-heal | src/dsp/AnamorphEngine.cpp:1863-1913 |
| 7 · Bypass crossfade | src/dsp/AnamorphEngine.cpp:1915-1940 (`bypassBlend`) |
| 8 · Metering tap | src/dsp/AnamorphEngine.cpp:1942-1951 (`scope.pushBlock`) |

## Invariants (must hold; each is testable)

| Invariant | Where enforced | Test |
|---|---|---|
| **Mono Maker runs POST-Mix**, on the mixed signal, in place. | src/dsp/AnamorphEngine.cpp:1715 (`monoMaker.process`) | testMonoMakerPostMix |
| **Band Solo is the very last audio stage and is monitoring-only** — it never changes any effect stage; `mask==0` → bit-exact true output. | src/dsp/AnamorphEngine.cpp:1861 (`soloMonitor.process`); src/dsp/SoloMonitor.h:12-16, 33 | testSoloMonitor, testSoloNoGhostInSilence |
...
| **Dry path is delay-compensated** to the wet (oversampling) latency. | src/dsp/AnamorphEngine.cpp:1600-1708, getLatencySamples | testBypassNullAndLatency |
| **Dry path is phase-matched** through the same crossovers as the wet (A(dry)) so a partial Mix never combs the mono sum. The reconstruction is gated off in the settled-full-wet state (Mix exactly 1, Match off, no crossfade — Wave 2 / H4) and re-engages phase-matched on a Mix dip. | src/dsp/AnamorphEngine.cpp:1479-1598, 1600-1708 | testMultibandMonoCompat, testDryAlignGateRecomb |
| **Mix = 0 is a bit-exact null** (smoothstep clean→aligned crossfade over first ~5% of Mix). | src/dsp/AnamorphEngine.cpp:1613, 1647-1667 (`kAlignMix`) | testBypassNullAndLatency / testTransparentDefault |
...
| **Bypass is a click-free crossfade to the delay-aligned RAW input**, not a mute; chain + analysis always run. | src/dsp/AnamorphEngine.cpp:1207-1278, 1915-1940 (`bypassBlend`) | testBypassCrossfadeClickFree, testLevelMatchRunsInBypass |
| **Level Match measures the post-Mono-Maker output vs the delay-aligned reconstruction A(dry).** | src/dsp/AnamorphEngine.cpp:1727-1732 (`loudness.process`) | testLevelMatchUnity, testMultibandUnityMatch |
...
- **M/S domain.** When `msMode` is on, Input conditioning decodes Mid/Side→L/R inside
  ...
  Source: src/dsp/AnamorphEngine.cpp:838-900 (`applyInputConditioning`); the M/S
  decode branch is src/dsp/AnamorphEngine.cpp:875-888.
- **Discrete switches** (algorithm/routing/band-count/oversampling-path) are applied at the
  ...
  **`dimMode` move while neither side is Dimension D**: that value is read by one line inside the
  ...
  Source: src/dsp/AnamorphEngine.cpp:405-468 (`discreteDiffers`); the duck fade
  times at src/dsp/AnamorphEngine.cpp:87-88 (`switchIncOut`); the Multiband Enable
  crossfade at src/dsp/AnamorphEngine.cpp:1494-1598 (`mbEnableBlend`) — which since ADR-0005's
  ...
  src/dsp/AnamorphEngine.cpp:1861 (`soloMonitor.process`); the Bypass crossfade at
  src/dsp/AnamorphEngine.cpp:1915-1940 (`bypassBlend`).
