...
- Source: src/dsp/AnamorphEngine.cpp:1227-2197 (`process`)
...
| Stage | Source |
|---|---|
| 0 · Input level tap | src/dsp/AnamorphEngine.cpp:1444 (`levels.input.process`) |
| 0b · True-bypass dry capture | src/dsp/AnamorphEngine.cpp:1446-1517 (`bypassDelayBuffer`) |
| 1 · Input conditioning | src/dsp/AnamorphEngine.cpp:1520 (`applyInputConditioning`) |
| 1b · M/S Solo | src/dsp/AnamorphEngine.cpp:1525-1528 |
| — conditioned-input capture | src/dsp/AnamorphEngine.cpp:1536-1537 (`dryScratch`) |
| 2a · Oversampled nonlinear region | src/dsp/AnamorphEngine.cpp:1539-1696 |
| 2b · Linear algorithm at base rate | src/dsp/AnamorphEngine.cpp:1698-1700 |
| 2c · Global Width (MS-domain) | src/dsp/AnamorphEngine.cpp:1702-1716 (`applyWidth`) |
| 2d · Multiband Width | src/dsp/AnamorphEngine.cpp:1718-1837 (`multiband.processBlock`) |
| 3 · Dry/Wet Mix | src/dsp/AnamorphEngine.cpp:1839-1947 |
| 4 · Mono Maker (post-Mix) | src/dsp/AnamorphEngine.cpp:1954 (`monoMaker.process`) |
| 5 · Output stage | src/dsp/AnamorphEngine.cpp:1956-2088 |
| 6 · Band Solo monitor | src/dsp/AnamorphEngine.cpp:2106 (`soloMonitor.process`) |
| 6b · NaN/Inf self-heal | src/dsp/AnamorphEngine.cpp:2108-2158 |
| 7 · Bypass crossfade | src/dsp/AnamorphEngine.cpp:2160-2185 (`bypassBlend`) |
| 8 · Metering tap | src/dsp/AnamorphEngine.cpp:2187-2196 (`scope.pushBlock`) |

## Invariants (must hold; each is testable)

| Invariant | Where enforced | Test |
|---|---|---|
| **Mono Maker runs POST-Mix**, on the mixed signal, in place. | src/dsp/AnamorphEngine.cpp:1954 (`monoMaker.process`) | testMonoMakerPostMix |
| **Band Solo is the very last audio stage and is monitoring-only** — it never changes any effect stage; `mask==0` → bit-exact true output. | src/dsp/AnamorphEngine.cpp:2106 (`soloMonitor.process`); src/dsp/SoloMonitor.h:12-16, 33 | testSoloMonitor, testSoloNoGhostInSilence |
...
| **Dry path is delay-compensated** to the wet (oversampling) latency. | src/dsp/AnamorphEngine.cpp:1839-1947, getLatencySamples | testBypassNullAndLatency |
| **Dry path is phase-matched** through the same crossovers as the wet (A(dry)) so a partial Mix never combs the mono sum. The reconstruction is gated off in the settled-full-wet state (Mix exactly 1, Match off, no crossfade — Wave 2 / H4) and re-engages phase-matched on a Mix dip. | src/dsp/AnamorphEngine.cpp:1718-1837, 1839-1947 | testMultibandMonoCompat, testDryAlignGateRecomb |
| **Mix = 0 is a bit-exact null** (smoothstep clean→aligned crossfade over first ~5% of Mix). | src/dsp/AnamorphEngine.cpp:1852, 1886-1906 (`kAlignMix`) | testBypassNullAndLatency / testTransparentDefault |
...
| **Bypass is a click-free crossfade to the delay-aligned RAW input**, not a mute; chain + analysis always run. | src/dsp/AnamorphEngine.cpp:1446-1517, 2160-2185 (`bypassBlend`) | testBypassCrossfadeClickFree, testLevelMatchRunsInBypass |
| **Level Match measures the post-Mono-Maker output vs the delay-aligned reconstruction A(dry).** | src/dsp/AnamorphEngine.cpp:1966-1971 (`loudness.process`) | testLevelMatchUnity, testMultibandUnityMatch |
...
- **M/S domain.** When `msMode` is on, Input conditioning decodes Mid/Side→L/R inside
  ...
  Source: src/dsp/AnamorphEngine.cpp:1041-1103 (`applyInputConditioning`); the M/S
  decode branch is src/dsp/AnamorphEngine.cpp:1078-1091.
- **Discrete switches** (algorithm/routing/band-count/oversampling-path) are applied at the
  ...
  **`dimMode` move while neither side is Dimension D**: that value is read by one line inside the
  ...
  crossfade at src/dsp/AnamorphEngine.cpp:1733-1837 (`mbEnableBlend`) — which since ADR-0005's
  ...
  src/dsp/AnamorphEngine.cpp:2106 (`soloMonitor.process`); the Bypass crossfade at
  src/dsp/AnamorphEngine.cpp:2160-2185 (`bypassBlend`).
