...
- Source: src/dsp/AnamorphEngine.cpp:759-1514 (`process`)
...
| Stage | Source |
|---|---|
| 0 · Input level tap | src/dsp/AnamorphEngine.cpp:910 (`levels.input.process`) |
| 0b · True-bypass dry capture | src/dsp/AnamorphEngine.cpp:912-983 (`bypassDelayBuffer`) |
| 1 · Input conditioning | src/dsp/AnamorphEngine.cpp:986 (`applyInputConditioning`) |
| 1b · M/S Solo | src/dsp/AnamorphEngine.cpp:991-994 |
| — conditioned-input capture | src/dsp/AnamorphEngine.cpp:1002-1003 (`dryScratch`) |
| 2a · Oversampled nonlinear region | src/dsp/AnamorphEngine.cpp:1005-1052 |
| 2b · Linear algorithm at base rate | src/dsp/AnamorphEngine.cpp:1054-1056 |
| 2c · Global Width (MS-domain) | src/dsp/AnamorphEngine.cpp:1058-1072 (`applyWidth`) |
| 2d · Multiband Width | src/dsp/AnamorphEngine.cpp:1074-1160 (`multiband.processBlock`) |
| 3 · Dry/Wet Mix | src/dsp/AnamorphEngine.cpp:1162-1270 |
| 4 · Mono Maker (post-Mix) | src/dsp/AnamorphEngine.cpp:1277 (`monoMaker.process`) |
| 5 · Output stage | src/dsp/AnamorphEngine.cpp:1279-1405 |
| 6 · Band Solo monitor | src/dsp/AnamorphEngine.cpp:1423 (`soloMonitor.process`) |
| 6b · NaN/Inf self-heal | src/dsp/AnamorphEngine.cpp:1425-1475 |
| 7 · Bypass crossfade | src/dsp/AnamorphEngine.cpp:1477-1502 (`bypassBlend`) |
| 8 · Metering tap | src/dsp/AnamorphEngine.cpp:1504-1513 (`scope.pushBlock`) |

## Invariants (must hold; each is testable)

| Invariant | Where enforced | Test |
|---|---|---|
| **Mono Maker runs POST-Mix**, on the mixed signal, in place. | src/dsp/AnamorphEngine.cpp:1277 (`monoMaker.process`) | testMonoMakerPostMix |
| **Band Solo is the very last audio stage and is monitoring-only** — it never changes any effect stage; `mask==0` → bit-exact true output. | src/dsp/AnamorphEngine.cpp:1423 (`soloMonitor.process`); src/dsp/SoloMonitor.h:12-16, 33 | testSoloMonitor, testSoloNoGhostInSilence |
...
| **Dry path is delay-compensated** to the wet (oversampling) latency. | src/dsp/AnamorphEngine.cpp:1162-1270, getLatencySamples | testBypassNullAndLatency |
| **Dry path is phase-matched** through the same crossovers as the wet (A(dry)) so a partial Mix never combs the mono sum. The reconstruction is gated off in the settled-full-wet state (Mix exactly 1, Match off, no crossfade — Wave 2 / H4) and re-engages phase-matched on a Mix dip. | src/dsp/AnamorphEngine.cpp:1074-1160, 1162-1270 | testMultibandMonoCompat, testDryAlignGateRecomb |
| **Mix = 0 is a bit-exact null** (smoothstep clean→aligned crossfade over first ~5% of Mix). | src/dsp/AnamorphEngine.cpp:1175, 1209-1229 (`kAlignMix`) | testBypassNullAndLatency / testTransparentDefault |
...
| **Bypass is a click-free crossfade to the delay-aligned RAW input**, not a mute; chain + analysis always run. | src/dsp/AnamorphEngine.cpp:912-983, 1477-1502 (`bypassBlend`) | testBypassCrossfadeClickFree, testLevelMatchRunsInBypass |
| **Level Match measures the post-Mono-Maker output vs the delay-aligned reconstruction A(dry).** | src/dsp/AnamorphEngine.cpp:1289-1294 (`loudness.process`) | testLevelMatchUnity, testMultibandUnityMatch |
...
- **M/S domain.** When `msMode` is on, Input conditioning decodes Mid/Side→L/R inside
  ...
  Source: src/dsp/AnamorphEngine.cpp:603-665 (`applyInputConditioning`); the M/S
  decode branch is src/dsp/AnamorphEngine.cpp:640-653.
- **Discrete switches** (algorithm/routing/band-count/oversampling-path) are applied at the
  ...
  Source: src/dsp/AnamorphEngine.cpp:276-307 (`discreteDiffers`); the duck fade
  times at src/dsp/AnamorphEngine.cpp:82-83 (`switchIncOut`); the Multiband Enable
  crossfade at src/dsp/AnamorphEngine.cpp:1089-1160 (`mbEnableBlend`); Band Solo at
  src/dsp/AnamorphEngine.cpp:1423 (`soloMonitor.process`); the Bypass crossfade at
  src/dsp/AnamorphEngine.cpp:1477-1502 (`bypassBlend`).
