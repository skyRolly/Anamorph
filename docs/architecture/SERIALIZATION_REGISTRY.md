...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2551-2554 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2366-2374 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PluginProcessor.cpp:2306
...
src/PluginProcessor.cpp:2412-2519; State test 27.
...
src/PluginProcessor.cpp:2692-2726 (the else-branch), :607 (the unparsable-blob guard).
...
src/PresetManager.cpp:809-862 (`encodeSelection` / `decodeSelection`);
src/PluginProcessor.cpp:2003-2025 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2665-2689 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2575-2659
(`readSlot`), :904-932 (`abEnsureInit`); src/PluginProcessor.h:501-514 (`StateSet::isValid`).
...
producer fills it — and it is resolved at **decode**, from the slot's own bytes, the same way the
ROOT `presetBaseline` is resolved (ADR-0036 §22): `readSlot` sets the baseline of a valid slot to
`PresetManager::soundSignatureAfterRestoring` over its parsed params, the signature of the sound
those bytes install (ADR-0037). A literal `""` would compare unequal to every possible `soundSig()`,
so such a slot would read as permanently edited and the top bar would render a bare ` *` — a
modified-marker against a preset the slot does not have (its name is empty by the same rule). Until
ADR-0037 the slot side kept a live-read fallback in `PresetManager::setMeta` (the state being applied
became its own clean baseline, read from the live parameters on the first switch-in); that fallback
...
Source: src/PresetManager.h:127-153 (`setMeta`);
src/PluginProcessor.cpp:2331 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2622-2623 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2691-2714.
