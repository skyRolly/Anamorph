...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2700-2703 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2515-2523 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:104-109 (`defaultName`); src/PluginProcessor.cpp:2455
...
src/PluginProcessor.cpp:2561-2668; State test 27.
...
src/PluginProcessor.cpp:2841-2875 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:2152-2174 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2814-2838 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2724-2808
(`readSlot`), :904-932 (`abEnsureInit`); src/PluginProcessor.h:539-552 (`StateSet::isValid`).
...
src/PluginProcessor.cpp:2480 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2771-2772 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2840-2863.
