...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2764-2767 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2579-2587 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:104-109 (`defaultName`); src/PluginProcessor.cpp:2519
...
src/PluginProcessor.cpp:2625-2732; State test 27.
...
src/PluginProcessor.cpp:2905-2939 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:2216-2238 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2878-2902 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2788-2872
...
src/PluginProcessor.cpp:2544 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2835-2836 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2904-2927.
