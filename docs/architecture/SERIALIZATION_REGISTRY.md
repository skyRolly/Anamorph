...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2766-2769 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2581-2589 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:104-109 (`defaultName`); src/PluginProcessor.cpp:2521
...
src/PluginProcessor.cpp:2627-2734; State test 27.
...
src/PluginProcessor.cpp:2907-2941 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:2218-2240 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2880-2904 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2790-2874
...
src/PluginProcessor.cpp:2546 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2837-2838 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2906-2929.
