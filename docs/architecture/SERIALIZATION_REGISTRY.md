...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:784-787 (pre-0.8.4 `migrateFromLegacyApvts`), :652-693 (pre-0.6.4 `readSlot`), :700-705 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:733-734 (write), :633-642 (read), :727-741 (adoption).
...
src/PluginProcessor.cpp:874-900 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:706-727 (`writeSelection`/`readSelection`), :585 (root write),
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:808-864
...
Evidence [Verified]: src/PluginProcessor.cpp:864-865 (the legacy-key fallback inside `readSlot`, :740-793);
...
Source: src/PluginProcessor.cpp:868-873.
