...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:1555-1558 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:1371-1379 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PluginProcessor.cpp:1311
...
src/PluginProcessor.cpp:1411-1518; State test 27.
...
src/PluginProcessor.cpp:1681-1717 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:1106-1128 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:1654-1678 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:1579-1652
(`readSlot`), :904-932 (`abEnsureInit`); src/PluginProcessor.h:215-228 (`StateSet::isValid`).
...
src/PluginProcessor.cpp:1336 (`baselineOfRestore`, the root-side rule).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:1626-1627 (the legacy-key fallback inside `readSlot`, :991-1078);
...
Source: src/PluginProcessor.cpp:1680-1705.
