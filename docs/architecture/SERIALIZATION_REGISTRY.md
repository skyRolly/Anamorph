...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:1570-1573 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:1385-1393 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PluginProcessor.cpp:1325
...
src/PluginProcessor.cpp:1431-1538; State test 27.
...
src/PluginProcessor.cpp:1711-1745 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:1110-1132 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:1684-1708 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:1594-1678
(`readSlot`), :904-932 (`abEnsureInit`); src/PluginProcessor.h:256-269 (`StateSet::isValid`).
...
src/PluginProcessor.cpp:1350 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:1641-1642 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:1710-1733.
