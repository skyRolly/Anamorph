...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:1450-1453 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:1271-1279 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PresetManager.cpp:658-670
(`adoptRestoredState`).
...
src/PluginProcessor.cpp:1311-1418; State test 27.
...
src/PluginProcessor.cpp:1576-1611 (the else-branch), :607 (the unparsable-blob guard).
...
src/PresetManager.cpp:676-729 (`encodeSelection` / `decodeSelection`);
...
Source: src/PluginProcessor.cpp:1549-1573 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:1474-1547
...
src/PresetManager.cpp:658-670 (`adoptRestoredState`, the root-side rule).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:1521-1522 (the legacy-key fallback inside `readSlot`, :991-1078);
...
Source: src/PluginProcessor.cpp:1575-1599.
