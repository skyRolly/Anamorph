...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:1740-1743 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:1555-1563 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PluginProcessor.cpp:1495
...
src/PluginProcessor.cpp:1601-1708; State test 27.
...
src/PluginProcessor.cpp:1881-1915 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:1280-1302 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:1854-1878 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:1764-1848
...
src/PluginProcessor.cpp:1520 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:1811-1812 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:1880-1903.
