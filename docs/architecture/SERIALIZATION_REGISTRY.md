...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2770-2773 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2585-2593 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:104-109 (`defaultName`); src/PluginProcessor.cpp:2525
...
src/PluginProcessor.cpp:2631-2738; State test 27.
...
src/PluginProcessor.cpp:2911-2945 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:2222-2244 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2884-2908 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2794-2878
...
src/PluginProcessor.cpp:2550 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2841-2842 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2910-2933.
