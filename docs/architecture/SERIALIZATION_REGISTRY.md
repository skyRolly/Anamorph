...
Evidence [Verified]: backward-compat paths at src/PluginProcessor.cpp:2241-2244 (pre-0.8.4, `resolveLegacy`), :1287-1360 (pre-0.6.4 `readSlot`), :1388-1410 (v0.2 bare APVTS);
...
Source: src/PluginProcessor.cpp:2056-2064 (write), :1268-1277 (read), :1104-1105 (adoption).
...
Source: src/PresetManager.h:103-108 (`defaultName`); src/PluginProcessor.cpp:1996
...
src/PluginProcessor.cpp:2102-2209; State test 27.
...
src/PluginProcessor.cpp:2382-2416 (the else-branch), :607 (the unparsable-blob guard).
...
src/PluginProcessor.cpp:1723-1745 (`writeSelection`/`readSelection`), :585 (root write),
...
Source: src/PluginProcessor.cpp:2355-2379 (the `AB`-absent branch of `decodeRestore`), :1388-1410 (the
...
a later save wrote that duplicate out. Source: src/PluginProcessor.cpp:2265-2349
(`readSlot`), :904-932 (`abEnsureInit`); src/PluginProcessor.h:455-468 (`StateSet::isValid`).
...
src/PluginProcessor.cpp:2021 (`baselineOfRestore`, the root-side rule); :1555-1639 (`readSlot`, the
derivation at its end).

**◊** Pre-0.6.4 sessions stored params-only under `slotA`/`slotB`; `readSlot` migrates them.
Evidence [Verified]: src/PluginProcessor.cpp:2312-2313 (the legacy-key fallback inside `readSlot`, :1555-1639);
...
Source: src/PluginProcessor.cpp:2381-2404.
