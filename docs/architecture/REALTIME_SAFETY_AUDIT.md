...
| `AnamorphEngine::process` | **Verified** — all scratch pre-sized; no alloc/lock/IO | prepare(): all buffers + oversamplers | src/dsp/AnamorphEngine.cpp:43-196 vs :660-1339 |
...
- **Non-finite guard:** an engine-wide per-sample NaN/Inf check replaces only non-finite
  ...
  audio. Evidence: src/dsp/AnamorphEngine.cpp:1977-2027.
...
Source for the OS init: src/dsp/AnamorphEngine.cpp:78-80 (`initProcessing` at prepare).
...
discrete change — `src/dsp/AnamorphEngine.cpp:1143-1282`: algorithm tails cleared, the three
...
target (`src/dsp/AnamorphEngine.cpp:220-226`), so every armed block sat in the steady-state
