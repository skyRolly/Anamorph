...
| `AnamorphEngine::process` | **Verified** — all scratch pre-sized; no alloc/lock/IO | prepare(): all buffers + oversamplers | src/dsp/AnamorphEngine.cpp:43-182 vs :660-1339 |
...
- **Non-finite guard:** an engine-wide per-sample NaN/Inf check replaces only non-finite
  ...
  audio. Evidence: src/dsp/AnamorphEngine.cpp:1943-1993.
...
discrete change — `src/dsp/AnamorphEngine.cpp:1120-1249`: algorithm tails cleared, the three
...
target (`src/dsp/AnamorphEngine.cpp:206-212`), so every armed block sat in the steady-state
