...
- **Non-finite guard:** an engine-wide per-sample NaN/Inf check replaces only non-finite
  ...
  audio. Evidence: src/dsp/AnamorphEngine.cpp:1838-1888.
...
discrete change — `src/dsp/AnamorphEngine.cpp:1031-1151`: algorithm tails cleared, the three
...
target (`src/dsp/AnamorphEngine.cpp:282-289`), so every armed block sat in the steady-state
