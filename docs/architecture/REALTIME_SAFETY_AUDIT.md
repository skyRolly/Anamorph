...
| `MultibandWidth` | **Verified** — fixed-coefficient dual-bank crossfade (0.8.10: coeffs recomputed once per ~12 ms fade, 2× filter ticks only while fading), no alloc/lock/IO | prepare(): scalar only (24× `LR4Xover.prepare`, flat state — no heap since Wave 2 / H6) | MultibandWidth.cpp (prepare/reset/fade trigger/processBlock) |
| `SoloMonitor` | **Verified** — fixed-coefficient dual-bank crossfade (0.8.10) + `SmoothedValue`, no alloc | prepare(): 6× flat-state filter + smoother reset | SoloMonitor.cpp (prepare/reset/fade trigger/process) |
