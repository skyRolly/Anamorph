...
| `AnamorphEngine::process` | **Verified** — all scratch pre-sized; no alloc/lock/IO | prepare(): all buffers + oversamplers | src/dsp/AnamorphEngine.cpp:43-198 vs :660-1339 |
...
- **Non-finite guard:** an engine-wide per-sample NaN/Inf check replaces only non-finite
  ...
  audio. Evidence: src/dsp/AnamorphEngine.cpp:1979-2029.
...
discrete change — `src/dsp/AnamorphEngine.cpp:1145-1284`: algorithm tails cleared, the three
...
target (`src/dsp/AnamorphEngine.cpp:222-228`), so every armed block sat in the steady-state
