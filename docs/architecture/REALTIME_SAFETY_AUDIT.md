...
| `MultibandWidth` | **Verified** — hybrid cutoff moves (0.8.10: per-sample one-pole coeff recompute while gliding; one ~12 ms dual-bank crossfade with 2× filter ticks on a multi-octave jump), no alloc/lock/IO | prepare(): scalar only (24× `LR4Xover.prepare`, flat state — no heap since Wave 2 / H6) | MultibandWidth.cpp (prepare/reset/glide + fade trigger/processBlock) |
| `SoloMonitor` | **Verified** — hybrid cutoff moves (0.8.10, as MultibandWidth) + `SmoothedValue`, no alloc | prepare(): 6× flat-state filter + smoother reset | SoloMonitor.cpp (prepare/reset/glide + fade trigger/process) |
