...
| `AnamorphEngine::process` | **Verified** — all scratch pre-sized; no alloc/lock/IO | prepare(): all buffers + oversamplers | src/dsp/AnamorphEngine.cpp:28-127 vs :660-1339 |
...
- **Non-finite guard:** an engine-wide per-sample NaN/Inf check replaces only non-finite
  ...
  audio. Evidence: src/dsp/AnamorphEngine.cpp:1312-1356.
...
discrete change — `src/dsp/AnamorphEngine.cpp:727-802`: algorithm tails cleared, the three
...
target (`src/dsp/AnamorphEngine.cpp:152-159`), so every armed block sat in the steady-state
