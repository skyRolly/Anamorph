...
| **AAX** | **Not Supported** | Out of scope: needs an Avid account + PACE/iLok signing. README:354. (DSP core is wrapper-agnostic, so a future AAX wrapper is low-cost, but it is explicitly not built today.) |
...
| Host | Status | Note |
|---|---|---|
| REAPER | **Unverified** | Referenced for its parameter-listing behaviour (drove the 0.8.4 InternalState design) and as the suggested audition host; not a tested compatibility claim. README:506; src/InternalState.h:15-18 |
