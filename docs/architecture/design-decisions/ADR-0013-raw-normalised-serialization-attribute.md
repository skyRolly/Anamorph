...
2. On restore, `reassertParameters()` re-applies each parameter from `raw` when present, else from the
   denormalised `value`.
3. Discrete parameters are reimplemented as minimal from-scratch `juce::RangedAudioParameter`
   subclasses (`RawChoice`/`RawBool`/`RawInt`, anonymous namespace in `PluginParameters.cpp`) whose
   `getValue()` returns the exact raw normalised value. The stock `AudioParameterBool/Choice/Int`
   *cannot* be subclassed for this — JUCE declares their `getValue()`/`setValue()` **private**, so an
   override-and-forward template (`RawValued<Base>`) fails to compile. The DSP still reads the snapped
   value via `getRawParameterValue()` (APVTS stores `range.convertFrom0to1(getValue())`); host text
   snaps via `getText()`/`getAllValueStrings()`.
...
- **C. Additive `raw` attribute + `reassertParameters` + from-scratch discrete params.** Chosen —
  additive, compatible, deterministic, and does not alter DSP, parameter IDs, ranges, defaults, or
  choice order.
...
- Discrete params are no longer `AudioParameterBool/Choice/Int`, so the two places that reached for
  those concrete types were re-pointed at the base pointer / a generic accessor: `getBypassParameter()`
  holds an `AudioProcessorParameter*` (no `dynamic_cast`), and the ComboBox item list is populated from
  `AudioProcessorParameter::getAllValueStrings()`. Behaviour is unchanged; only the access path is generic.
...
- Source: `src/PluginProcessor.cpp` (`copyStateWithRawValues` / `reassertParameters`);
  `src/PluginParameters.cpp` (`RawChoice` / `RawBool` / `RawInt`).
