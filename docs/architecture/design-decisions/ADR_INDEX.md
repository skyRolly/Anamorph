...
| [ADR-0028](ADR-0028-clang-toolchain-pin.md) | CI Clang toolchain pin 18 → 20, held as a **major-only** pin (stock Ubuntu archives, no `apt.llvm.org`; 19/21/22, a full-version pin and unpinning each evaluated and rejected; governs two jobs that ship nothing; deliberately not bot-updatable) | **Proposed** (Build System change by the ADR-0027 reading — awaiting Architecture Review) | Verified (clang-18 control reproduces the committed baseline byte-identically; `diff`-identical 52-instance warning census 18 vs 20 so only `# clang-major:` moved; 140 + 894 suites green under the clang-20 build and again under ASan+UBSan; compile-canary still rejects the explicit form; the mismatch guard observed firing in both directions) |
