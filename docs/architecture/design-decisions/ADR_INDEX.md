...
| [ADR-0027](ADR-0027-cxx23-language-standard.md) | C++ language standard 17 → 23 (`CMAKE_CXX_EXTENSIONS OFF` retained; one `<algorithm>` include added; no version bump; C++20 fallback evaluated and not taken) | **Proposed** (Build System change — flagged on the PR for Architecture Review) | Verified (three-platform CI builds incl. AppleClang + MSVC, plus a local libc++ build; 32-scenario twin-dump bit-identical incl. latencies C++17 vs C++23; 140 + 894 suites green under libstdc++ and libc++; identical 29-instance warning set; pluginval 10 both modes ×3) |
