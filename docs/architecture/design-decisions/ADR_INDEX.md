...
| [ADR-0024](ADR-0024-runtime-only-preset-identity.md) | Factory-preset identity is an immutable internal id (user presets are identified by their file), and the identity is **deliberately never serialized** — persisting it would be a Serialization Registry addition, i.e. an Architecture Review Gate item; a restored session therefore resolves a clashing name to the factory preset, as a documented tie-break | **Accepted** | Verified (code + state test 10; state test 1 unchanged proves the saved schema did not move) |
