...
| [ADR-0023](ADR-0023-vendor-manufacturer-code.md) | Vendor manufacturer code `Anmf` → `RTec` (product-line identity; AU component + VST3 UID change, pre-0.9.1 sessions report the plug-in as missing — KI-016) | Proposed (pending Architecture-Review sign-off + the Level-5 host/`auval` re-registration check) | Verified (code) / Unverified (host behaviour — no automated gate observes plug-in identity) |
