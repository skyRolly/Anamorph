...
| Claim | Evidence |
|---|---|
| **`processBlock` performs no heap allocations.** All scratch buffers and DSP state are allocated in `prepare()`; the audio path uses only pre-sized buffers and scalar state. | DSP audit of all 12 modules; src/dsp/AnamorphEngine.cpp:41-156 (prepare allocations) vs the whole of `process()` (no `new`/resize/lock — line range re-verified Wave 3 after the function grew) |
| **No locks / mutexes / file IO on the audio thread.** | `REALTIME_SAFETY_AUDIT.md`; no `mutex`/`lock`/IO in `src/dsp/**` audio paths |
| **`ScopedNoDenormals` is active for the whole block.** | src/PluginProcessor.cpp:269 |
| **Oversampling only runs when nonlinear work exists** (Drive>0 or Chorus/Dim-D); linear chains skip it → no needless CPU. **Unchanged by ADR-0034 and ADR-0035.** ADR-0034 decoupled the reported LATENCY from this predicate without touching the run gate: where the wrap is skipped, a 2-channel integer ring supplies its group delay instead. ADR-0035 turned the engage/disengage into a 12 ms crossfade, so the wrap additionally runs for the duration of a crossing — and goes cold again once the blend has left it, which is what keeps the saving. Re-measured after both, at 48 kHz / 128 on the `working` chain with Drive 0 and a linear algorithm — OS Off 159.19, 2× 170.06, 4× 166.60, 8× 169.44 ns/sample, i.e. inside each other's 5–9 % run-to-run spread and far below the engaged rows (210.44 / 266.13 / 375.85). The ring's own cost, and the always-on write that keeps it warm, are below this instrument's floor. | src/dsp/AnamorphEngine.cpp (`osActiveFor`, `osCompDelayBuffer`); `AnamorphBench` §"Oversampling SELECTED but skipped, Drive 0" |
...
and `maxBlockSize` at prepare() time (src/dsp/AnamorphEngine.cpp:41-44) with no hard-coded SR
