...
| RH-F3 | **`auval` AU conformance in macOS CI** | **Defer — add after the tag, non-blocking first** | The AU is a shipped format with **zero** automated validation (pluginval only sees the VST3). The recipe is known — copy the built `.component` into `~/Library/Audio/Plug-Ins/Components/`, `killall -9 AudioComponentRegistrar`, then `auval -v aufx Anmr Anmf` — but its reliability on a headless `macos-14` runner (window server availability for auval's view test, registry timing) cannot be established from this repository, and introducing an unproven **blocking** gate immediately before a release tag is the wrong trade. Land it after v0.9.0, run it non-blocking for several builds, then promote. See `docs/procedures/TESTING.md` §"Gaps in the automated coverage". |
