...
| Symbol hygiene | **Closed by RH-PR-2 (ADR-0021) on two of three platforms:** retain-then-strip pipeline everywhere — debug info generated (`-g`/`/Zi`+`/DEBUG`), captured as separate `Anamorph-<OS>-debug` artifacts (split `.debug` on Linux, PDB on Windows), public binaries stripped (`nm: no symbols`; dynamic exports untouched). **macOS is the exception and it is not a partial one: under Release+LTO no usable dSYM is produced at all** (see the dSYM row below), so `Anamorph-macOS-debug` is normally absent rather than occasionally so. Visibility was already hidden via JUCE's plugin helpers (JUCEUtils.cmake) — the earlier "no explicit `CXX_VISIBILITY_PRESET`" row described our CMakeLists, not the effective build; recorded in the ADR instead of restated | [Verified] build.yml, CMakeLists.txt (AnamorphHardening), RH_PR2_INVESTIGATION.md |
...
| QA gate | 37 DSP self-tests + A/B guard (160 checks) + the 13-test state-compatibility suite (900 checks) + pluginval strictness 10, deterministic + randomise ×3, blocking on 3 OSes; Level-5 manual audition | [Verified] TESTING_POLICY.md, CI_CD.md |
...
| RH-R8 | No crash reporting; ~~no retained debug symbols~~ **symbol retention shipped in RH-PR-2 for Linux and Windows** (per-run `Anamorph-<OS>-debug` artifacts: split `.debug`, PDB). **macOS symbolication is NOT delivered today** — Release+LTO leaves the DWARF in a linker temporary that is gone before `dsymutil` runs, so the validated-dSYM condition never holds and the upload is skipped every run. A full crash-reporter remains Phase-2 (§7) | Medium → **Low on Linux/Windows, unchanged on macOS** (which is the platform whose users most often submit an OS crash log) |
...
| Strip | Release artifacts stripped (`strip -x` mac / `strip --strip-unneeded` Linux / PDB separated on MSVC), **with split-debuginfo and PDB generated first and retained as CI artifacts** for crash symbolication (pairs with RH-R8) | **Implemented (RH-PR-2, ADR-0021)**; **macOS dSYM open** — see below |
| macOS dSYM | Capture is best-effort and, in this build configuration, **never succeeds**: `juce_recommended_lto_flags` is linked into the shipped target, so under Release+LTO the DWARF the linked binary references lives in ld64's temporary LTO object, which is deleted before `dsymutil` runs. `dsymutil` then warns, exits 0, and emits a dSYM with no usable DWARF; the packaging step correctly discards it (it validates the OUTPUT — DWARF payload present, UUID set matching every slice, ≥1 compile unit — rather than the warning text) and skips the `-debug` upload with a `::warning::`. The customer pipeline is never blocked by this. **The fix is one linker flag** — `-Wl,-object_path_lto=<dir>` on the macOS link, which tells ld64 to write that object to a path that persists instead of a temp file; it changes where a temporary is written, not what is linked, so the shipped bytes are unaffected. It is nonetheless a **shipped-link change on the release job that cannot be validated anywhere but macOS**, so it belongs in a change that can run that job and inspect the resulting dSYM, not in a Linux-authored batch | **Open (RH-PR-3 candidate).** Corrected 2026-08-18: the three rows above previously read as though symbolication were delivered on all three platforms |
