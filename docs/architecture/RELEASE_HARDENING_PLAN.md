...
| QA gate | 36 DSP self-tests + A/B guard (156 checks) + the 13-test state-compatibility suite (900 checks) + pluginval strictness 10, deterministic + randomise ×3, blocking on 3 OSes; Level-5 manual audition | [Verified] TESTING_POLICY.md, CI_CD.md |
...
| RH-F4 | **Golden-audio regression harness** | **Do not build — the twin-dump method is the right tool** | A committed golden waveform would freeze bit-exact output, colliding with the Class-B numerical changes `DSP_POLICY.md` permits. The existing 156-check suite already pins the behavioural invariants (exact nulls, click-freeness, spectral/pitch bounds, cold-path bit-identity). What is genuinely missing is that the **twin dump** (build before/after, run a scenario matrix through both, compare hashes + reported latencies — `worklogs/JUCE9_MIGRATION_v0.8.13.md`) is re-created per investigation instead of being a committed script. Promoting that harness to `scripts/` is the useful version of this item. |
