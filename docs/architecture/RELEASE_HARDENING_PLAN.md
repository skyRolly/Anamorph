...
| Version management | **Annotated `vX.Y.Z` tag convention adopted (RH-PR-8)**; no tag cut yet (first: **v0.9.3** — closes RISK-003 when practiced); version in `CMakeLists.txt` + About box; CI run number as build number | [Verified] RELEASE_PROCESS.md §Tagging |
...
| QA gate | 33 DSP self-tests + A/B guard (140 checks) + the 12-test state-compatibility suite (894 checks) + pluginval strictness 10, deterministic + randomise ×3, blocking on 3 OSes; Level-5 manual audition | [Verified] TESTING_POLICY.md, CI_CD.md |
...
| RH-R6 | ~~No git tags / tag-triggered release pipeline~~ **Pipeline shipped by RH-PR-8** (tag convention + `release.yml`); residual = no tag cut yet — closes with the first release tag (**v0.9.3**), which makes shipped bytes attributable to a source state (RISK-003) | Medium → Low |
...
blocks the first release tag — which is now **v0.9.3**, not v0.9.0, v0.9.1 or v0.9.2 (each was written up but never
...
| RH-F2 | **Steinberg VST 3 requirements review** (RH-R10 residual) | **Owner decision — blocks commercial sale** | The SDK *code* is MIT in JUCE 9.0.0, but VST trademark use and the plug-in development/distribution terms are governed separately (`VST3_Usage_Guidelines.pdf`; the SDK README's reference to a *Steinberg VST 3 Plug-In SDK Licensing Agreement*). No determination is made in this repository. |
| RH-F3 | **`auval` AU conformance in macOS CI** | **Defer — add after the tag, non-blocking first** | The AU is a shipped format with **zero** automated validation (pluginval only sees the VST3). The recipe is known — copy the built `.component` into `~/Library/Audio/Plug-Ins/Components/`, `killall -9 AudioComponentRegistrar`, then `auval -v aufx Anmr RTec` — but its reliability on a headless `macos-14` runner (window server availability for auval's view test, registry timing) cannot be established from this repository, and introducing an unproven **blocking** gate immediately before a release tag is the wrong trade. Land it after the first release tag (v0.9.3), run it non-blocking for several builds, then promote. See `docs/procedures/TESTING.md` §"Gaps in the automated coverage". |
| RH-F4 | **Golden-audio regression harness** | **Do not build — the twin-dump method is the right tool** | A committed golden waveform would freeze bit-exact output, colliding with the Class-B numerical changes `DSP_POLICY.md` permits. The existing 140-check suite already pins the behavioural invariants (exact nulls, click-freeness, spectral/pitch bounds, cold-path bit-identity). What is genuinely missing is that the **twin dump** (build before/after, run a scenario matrix through both, compare hashes + reported latencies — `worklogs/JUCE9_MIGRATION_v0.8.13.md`) is re-created per investigation instead of being a committed script. Promoting that harness to `scripts/` is the useful version of this item. |
