...
| Symbol hygiene | **Closed by RH-PR-2 (ADR-0021), on all three platforms since 2026-08-18:** retain-then-strip pipeline everywhere — debug info generated (`-g`/`/Zi`+`/DEBUG`), captured as separate `Anamorph-<OS>-debug` artifacts (split `.debug` on Linux, PDB on Windows, **dSYM on macOS**), public binaries stripped (`nm: no symbols`; dynamic exports untouched). macOS was the exception until `-Wl,-object_path_lto` landed — see the dSYM row below for what was wrong and how it was closed. Visibility was already hidden via JUCE's plugin helpers (JUCEUtils.cmake) — the earlier "no explicit `CXX_VISIBILITY_PRESET`" row described our CMakeLists, not the effective build; recorded in the ADR instead of restated | [Verified] build.yml, CMakeLists.txt (AnamorphHardening), RH_PR2_INVESTIGATION.md |
...
| Version management | **Annotated `vX.Y.Z` tag convention adopted (RH-PR-8)**; no tag cut yet (first: **v0.9.7** — closes RISK-003 when practiced); version in `CMakeLists.txt` + About box; CI run number as build number | [Verified] RELEASE_PROCESS.md §Tagging |
...
| QA gate | 52 DSP self-tests + A/B guard (364 checks) + the 35-test state-compatibility suite (1506 checks) + pluginval strictness 10, deterministic + randomise ×3, blocking on 3 OSes; Level-5 manual audition — performed against the final v0.9.6 build and PASSED (recorded 2026-09-01, `procedures/LEVEL5_AUDITION.md`), but **REOPENED for v0.9.7**: ADR-0034 changed the reported latency and one audible swap behaviour, and the audition is per-version with no carry-forward | [Verified] TESTING_POLICY.md, CI_CD.md |
...
| RH-R6 | ~~No git tags / tag-triggered release pipeline~~ **Pipeline shipped by RH-PR-8** (tag convention + `release.yml`); residual = no tag cut yet — closes with the first release tag (**v0.9.7**), which makes shipped bytes attributable to a source state (RISK-003) | Medium → Low |
...
| RH-R10 | **Third-party licence compliance is broader than the JUCE tier.** ~~Engineering half: no attribution file~~ **shipped (v0.9.0 hardening pass)**: [`NOTICE`](../../NOTICE) + [`THIRD_PARTY_LICENSES.md`](../../THIRD_PARTY_LICENSES.md) inventory every component compiled into the binaries, verified against the pinned JUCE tree, and discharge the three *mandatory* binary-distribution notices (IJG libjpeg, FLAC, Ogg Vorbis) plus HarfBuzz/SheenBidi. **Correction:** the VST3 SDK bundled with JUCE 9.0.0 is **MIT** (`VST3_SDK/LICENSE.txt`, © 2025 Steinberg Media Technologies GmbH), *not* the GPLv3/proprietary dual licence this row previously asserted — that described an older SDK release. What remains is the **owner half**: the VST trademark and the terms for developing/distributing VST 3 plug-ins are governed separately by Steinberg (the SDK ships `VST3_Usage_Guidelines.pdf`, and its README refers to a *Steinberg VST 3 Plug-In SDK Licensing Agreement* in the full SDK download). **Commercial VST3 distribution requires reviewing Steinberg's licensing requirements separately** — no determination is made in this repository. Found in the v0.8.13 product-readiness review; engineering deliverable closed in the v0.9.0 hardening pass | High → **Medium** (attribution done; the Steinberg review + the JUCE-tier/EULA decisions below still gate commercial sale) |
| RH-R11 | **No declared licence for Anamorph's own source or binaries.** The repository root has no `LICENSE` file and no EULA ships with the installers, so the terms on which Anamorph itself is offered are undeclared (`EULA.md` exists only as an unapproved draft — not in force, not shipped). The owner has stated the product model (2026-07-26): **closed-source commercial** — which rules out the AGPLv3 arm of the JUCE 9 dual licence, so the **commercial JUCE tier must be obtained** and Anamorph's own LICENSE/EULA text written before commercial distribution. Pure owner/legal actions; no engineering work can close them. Found in the v0.9.0 release-hardening audit | High (blocks a commercial release) |
...
| Strip | Release artifacts stripped (`strip -x` mac / `strip --strip-unneeded` Linux / PDB separated on MSVC), **with split-debuginfo, PDB and dSYM generated first and retained as CI artifacts** for crash symbolication (pairs with RH-R8) | **Implemented (RH-PR-2, ADR-0021; macOS completed 2026-08-18)** |
| macOS dSYM | **Closed 2026-08-18.** Capture was best-effort and, in this build configuration, never succeeded: `juce_recommended_lto_flags` is linked into the shipped target, so under Release+LTO the DWARF the linked binary references lived in ld64's temporary LTO object, which is deleted before `dsymutil` runs. `dsymutil` warned, exited 0, and emitted a dSYM with no usable DWARF; the packaging step correctly discarded it — every run. The fix is one linker flag, `-Wl,-object_path_lto=<dir>` on the macOS link (`AnamorphHardening`, `CMakeLists.txt`), which changes **where a temporary is written and not what is linked**: the bitcode, the codegen and the shipped bytes are untouched and LTO is still LTO. A directory rather than a file, because a universal build links once per architecture. **Verified on the macOS jobs rather than in source**: the build now asserts the retained-object directory is non-empty (a direct witness that ld64 ran LTO codegen), and the run that landed it produced three UUID-matched dSYMs and a 53 MB `Anamorph-macOS-debug` artifact where every previous run produced none. Zero usable dSYMs is an **error** rather than a warning, so a silent regression to no-symbolication cannot recur — raised as the macOS job's **final step**, after the customer pipeline and every upload, so a symbol regression fails the run and blocks the release (`release.yml` observes the aggregate result) without withholding the plug-in and installer that were built and validated on the way there | **Implemented** |
...
## 12a. Post-v0.9.0 follow-ups (from the release-hardening audit)
...
blocks the first release tag — which is now **v0.9.7**, none of v0.9.0 through v0.9.6 having been
...
|---|---|---|---|
| RH-F1 | **JUCE commercial licence + Anamorph's own LICENSE/EULA** (RH-R11) | **Owner action — blocks commercial sale** | The stated product model (closed-source commercial, 2026-07-26) rules out the AGPLv3 arm, so the commercial JUCE tier is required; obtaining it and approving Anamorph's own terms are owner/legal tasks, not engineering ones. The repository has no `LICENSE` file today, and `EULA.md` is an unapproved draft listing the ten decisions its approval needs. `THIRD_PARTY_LICENSES.md` §"Open licensing decisions" states the remaining action set precisely; `docs/COMMERCIAL_STATUS.md` §4 indexes all open owner/legal decisions. |
| RH-F2 | **Steinberg VST 3 requirements review** (RH-R10 residual) | **Owner decision — blocks commercial sale** | The SDK *code* is MIT in JUCE 9.0.1, but VST trademark use and the plug-in development/distribution terms are governed separately (`VST3_Usage_Guidelines.pdf`; the SDK README's reference to a *Steinberg VST 3 Plug-In SDK Licensing Agreement*). No determination is made in this repository. |
| RH-F3 | **`auval` AU conformance in macOS CI** | **Defer — add after the tag, non-blocking first** | **Premise superseded in v0.9.4, item narrowed rather than closed.** This row was written when the AU was a shipped format with **zero** automated validation; it is not one now — the macOS job runs the full pluginval release gate against the AU as well as the VST3 (same strictness, both modes ×3, against the packaged bundle), and the registry half of the recipe below turned out to be exactly what was needed: the `.component` is installed into `~/Library/Audio/Plug-Ins/Components/` with `AudioComponentRegistrar` restarted before validation. **KI-014 is therefore closed and removed from `KNOWN_ISSUES.md`.** What survives here is `auval` *specifically* — Apple's own conformance tool, which tests things pluginval does not — and its open question is unchanged: reliability on a headless `macos-latest` runner (window server availability for auval's view test, registry timing) cannot be established from this repository. That is now an ADDITION to a gate that exists rather than the only cover for a shipped format, so the trade is milder and the urgency lower: land it non-blocking, promote once stable. See `docs/procedures/CI_CD.md` §"Known coverage limits" and `docs/procedures/TESTING.md` §"Gaps in the automated coverage". |
| RH-F4 | **Golden-audio regression harness** | **Do not build — the twin-dump method is the right tool** | A committed golden waveform would freeze bit-exact output, colliding with the Class-B numerical changes `DSP_POLICY.md` permits. The existing 160-check suite already pins the behavioural invariants (exact nulls, click-freeness, spectral/pitch bounds, cold-path bit-identity). What is genuinely missing is that the **twin dump** (build before/after, run a scenario matrix through both, compare hashes + reported latencies — `worklogs/JUCE9_MIGRATION_v0.8.13.md`) is re-created per investigation instead of being a committed script. Promoting that harness to `scripts/` is the useful version of this item. |
| RH-F5 | **CPU / memory budget numbers (RISK-002)** | **Procedure documented; measurement outstanding** | `PERFORMANCE_BUDGET.md` now carries a required benchmark procedure (what to build, the sample-rate × block × algorithm × oversampling × multiband matrix, ns/sample + worst block, median of ≥5 runs, and the pass/fail question). Producing the numbers needs a quiet, known machine — not a CI runner and not this sandbox. |
| RH-F6 | **pluginval version is unpinned** | **Low — supply-chain follow-up** | `run-pluginval.sh` downloads `releases/latest`, so the release gate's own tool can change under the project. Pinning it is a `DEPENDENCY_POLICY.md`-shaped change, not a release blocker. |
