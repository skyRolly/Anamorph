...
|---|---|---|---|
| RH-F1 | **JUCE commercial licence + Anamorph's own LICENSE/EULA** (RH-R11) | **Owner action — blocks commercial sale** | The stated product model (closed-source commercial, 2026-07-26) rules out the AGPLv3 arm, so the commercial JUCE tier is required; obtaining it and writing Anamorph's own terms are owner/legal tasks, not engineering ones. The repository has no `LICENSE` file today. `THIRD_PARTY_LICENSES.md` §"Open licensing decisions" states the remaining action set precisely. |
