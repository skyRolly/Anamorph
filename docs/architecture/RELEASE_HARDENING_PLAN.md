...
| RH-R11 | **No declared licence for Anamorph's own source or binaries.** The repository root has no `LICENSE` file and no EULA ships with the installers, so the terms on which Anamorph itself is offered are undeclared — and the JUCE 9 dual licence (AGPLv3 *or* commercial) means that choice is coupled to the JUCE tier decision. Pure owner/legal decision; no engineering work can close it. Found in the v0.9.0 release-hardening audit | High (blocks a commercial release; a free/AGPL release still needs the file) |
...
|---|---|---|---|
| RH-F1 | **JUCE licence tier + Anamorph's own LICENSE/EULA** (RH-R11) | **Owner decision — blocks commercial sale** | JUCE 9 is AGPLv3-or-commercial; the tier choice and Anamorph's own terms are coupled, and neither is an engineering task. The repository has no `LICENSE` file today. `THIRD_PARTY_LICENSES.md` §"Open licensing decisions" states the decision set precisely. |
