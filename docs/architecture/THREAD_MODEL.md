...
Evidence [Verified]:
- Source: src/PluginProcessor.cpp:238-306 (`processBlock`), :240 `ScopedNoDenormals`
...
- Source: src/PluginEditor.cpp:1852-1853 (`triggerRepaint` guarded by `isAttached()`)
...
|---|---|---|---|
| `VBlankAttachment meterVBlank` | per display frame (dt clamped ≤ 0.05 s) | meter-reveal + micro-anims easing | src/PluginEditor.cpp:687-693 |
| Editor `juce::Timer` | 24 Hz | view-state sync, preset display, `pollUndoCoalesce()`, undo/redo enable, match-gain readout | src/PluginEditor.cpp:684, 1385-1548 |
...
(the VBlank lambda captures `this`). Source: src/PluginEditor.cpp:701-703.
