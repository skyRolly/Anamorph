...
| **Host prepare thread** (external) | host → `prepareToPlay` off the message thread (JUCE Linux VST3 pre-`IRunLoop`, FL Studio's Patcher, an AU `Initialize` off main) | Engine prepare under the format contract that no `processBlock` runs concurrently; latency through the D-1 request (round 15). It runs `reset (ResetScope::everything)`, so unlike the host-reset row below it DOES clear the display meters and the published `matchGainDb` — a new sample rate invalidates both. It leaves `prevPlaying` alone, and that is benign rather than an oversight: the hold has just been cleared outright, so a missed restart edge afterwards has nothing left to clear (State test 120, third leg). |
| **Host reset** (external) | host → `AudioProcessor::reset` — VST3 `setProcessing(false)` (`juce_audio_plugin_client_VST3.cpp:3475-3479`), AU `Reset()` (`juce_audio_plugin_client_AU_1.mm:255-263`); neither is followed by a `prepareToPlay` | `engine.reset()` under the same format contract as the prepare row above: no `processBlock` runs concurrently, so nothing is added to the audio path and no lock, wait or async hop is introduced. Touches engine DSP state only — no APVTS, no program metadata, and no latency write, so the reported latency stays parameter-driven (`PluginProcessor.cpp:41`, `:381`). Added R5/F2; before it the request reached nothing. **The scope rule, settled round 54:** a host reset clears AUDIO — delay lines, filter banks, rings, the in-flight duck and the Level-Match ANALYSIS (`loudness.softReset()`) — and leaves DISPLAY and the user's own latches alone: the published `matchGainDb` (ADR-0007) and the meters' held peak, which `LevelMeters.h` clears "on a number click or a playback restart" and on nothing else (`src/gui/LevelMeter.h:27`, `src/PluginProcessor.cpp:442`) — see the *Meter hold reset* row in the handoff table below. `correlation` and `levels` are therefore cleared under `ResetScope::everything` only. State test 120 asserts both halves; `scripts/check-state-coverage.py` holds the per-module decision in place. **And the lifecycle half, settled the same round:** the reset also clears `prevPlaying`, because it ENDS a processing session and that flag is a memory of one. The hold is cleared on the rising edge `playing && ! prevPlaying`, which needs a falling half — and the host class this override exists for sends none: VST3 `setProcessing(false)` calls `reset()` and then stops calling `process`, `setProcessing(true)` simply resumes, and neither re-prepares. Measured, a held −0.92 dB survived a stop-and-resume-in-place; it now clears at the RESTART, never at the stop. `prevPosValid` is left alone and is provably inert once `prevPlaying` is false. State test 121. |
