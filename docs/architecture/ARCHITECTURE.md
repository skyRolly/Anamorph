...
- Source: src/PluginProcessor.cpp:377-382 (per-block snapshot → `engine.setParameters` → `engine.process`)
...
- Source: src/PluginProcessor.cpp:327 (`juce::ScopedNoDenormals`)
...
Evidence [Verified]:
- Source: src/PluginProcessor.cpp:170-180 (`isBusesLayoutSupported`), :94-95 (mono→stereo upmix)
