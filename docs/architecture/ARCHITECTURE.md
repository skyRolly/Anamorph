...
- Source: src/PluginProcessor.cpp:425-430 (per-block snapshot → `engine.setParameters` → `engine.process`)
...
- Source: src/PluginProcessor.cpp:375 (`juce::ScopedNoDenormals`)
...
Evidence [Verified]:
- Source: src/PluginProcessor.cpp:198-208 (`isBusesLayoutSupported`), :94-95 (mono→stereo upmix)
