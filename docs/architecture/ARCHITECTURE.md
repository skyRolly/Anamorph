...
- Source: src/PluginProcessor.cpp:437-442 (per-block snapshot → `engine.setParameters` → `engine.process`)
...
- Source: src/PluginProcessor.cpp:387 (`juce::ScopedNoDenormals`)
...
Evidence [Verified]:
- Source: src/PluginProcessor.cpp:210-220 (`isBusesLayoutSupported`), :94-95 (mono→stereo upmix)
