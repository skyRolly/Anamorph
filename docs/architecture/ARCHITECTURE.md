...
- Source: src/PluginProcessor.cpp:357-362 (per-block snapshot → `engine.setParameters` → `engine.process`)
...
- Source: src/PluginProcessor.cpp:307 (`juce::ScopedNoDenormals`)
...
Evidence [Verified]:
- Source: src/PluginProcessor.cpp:150-160 (`isBusesLayoutSupported`), :94-95 (mono→stereo upmix)
