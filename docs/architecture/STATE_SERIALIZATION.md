...
Evidence [Verified]: src/PluginProcessor.cpp:2414-2500 (`getStateInformation`), :1226-1485
...
Evidence [Verified]: src/PluginProcessor.cpp:2414-2500 (`getStateInformation`).
...
| **pre-0.9.2**: no indicator identity in the session | `decodeSelection` yields `unknown` → name fallback | src/PresetManager.cpp:845-862; :128-131 |
...
Evidence [Verified]: src/PluginProcessor.cpp:911-967 (`applyStatePreservingView`).
