...
Evidence [Verified]: src/PluginProcessor.cpp:666-696 (`getStateInformation`), :605-744
...
Evidence [Verified]: src/PluginProcessor.cpp:666-696 (`getStateInformation`).
...
| **pre-0.9.2**: no indicator identity in the session | `decodeSelection` yields `unknown` → name fallback | src/PresetManager.cpp:389-406; :128-131 |
...
Evidence [Verified]: src/PluginProcessor.cpp:275-290 (`applyStatePreservingView`).
