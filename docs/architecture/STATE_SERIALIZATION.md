...
Evidence [Verified]: src/PluginProcessor.cpp:656-686 (`getStateInformation`), :605-744
...
Evidence [Verified]: src/PluginProcessor.cpp:656-686 (`getStateInformation`).
...
| **pre-0.9.2**: no indicator identity in the session | `decodeSelection` yields `unknown` → name fallback | src/PresetManager.cpp:381-398; :128-131 |
