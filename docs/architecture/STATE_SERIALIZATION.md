...
Evidence [Verified]: src/PluginProcessor.cpp:2658-2744 (`getStateInformation`).
...
| **pre-0.9.2**: no indicator identity in the session | `decodeSelection` yields `unknown` → name fallback | src/PresetManager.cpp:1315-1332; :128-131 |
...
Evidence [Verified]: src/PluginProcessor.cpp:1047-1122 (`applyStatePreservingView`).
