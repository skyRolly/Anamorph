...
Evidence [Verified]: src/PluginProcessor.cpp:841-871 (`getStateInformation`), :605-744
...
Evidence [Verified]: src/PluginProcessor.cpp:841-871 (`getStateInformation`).
...
| **pre-0.9.2**: no indicator identity in the session | `decodeSelection` yields `unknown` → name fallback | src/PresetManager.cpp:418-435; :128-131 |
...
Evidence [Verified]: src/PluginProcessor.cpp:354-369 (`applyStatePreservingView`).
