...
- **Likelihood (evidence-based):** **Low.** It requires the HOST to write cross-parameter from
  ...
  (`src/PluginProcessor.h:211-214`) is a single relaxed `fetch_add`,
  `ViewGenWatcher::parameterValueChanged` (`src/PluginProcessor.h:355`) the same, and
  `parameterGestureChanged` (`src/PluginProcessor.cpp:814-865`) touches two ints — the last
...
- **Risk:** `AnamorphAudioProcessor::parameterGestureChanged` counts open gestures and sets
  `pendingGestureCommit` when the count returns to zero (`src/PluginProcessor.cpp:814-864`), and
...
- **Evidence [Verified]:** `src/PluginProcessor.cpp:814-864` (the counter), `:827-834`
...
- **Risk (as recorded, now closed):** `getStateInformation`/`setStateInformation` mutate non-atomic message-thread-read
  ...
  `presets.setMeta`, `syncCommitted` (src/PluginProcessor.cpp:1790-1889 read
...
- **Round 21 (2026-09-02, ER-STATE-23 re-raised): re-measured on the current tree, same four
  ...
  (`setStateInformation`, `src/PluginProcessor.cpp:1790`) and with the same wording plus one added
  ...
  `src/PluginProcessor.cpp:1326`, against `canUndo()`; the `abUndo` vector's internals twice, via
  `UndoStacks::operator=` (`src/PluginProcessor.h:290`) against the reader's iteration; and the
