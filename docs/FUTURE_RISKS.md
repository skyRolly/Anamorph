...
- **Round 21 (2026-09-02, ER-STATE-23 re-raised): re-measured on the current tree, same four
  ...
  `src/PluginProcessor.cpp:1317`, against `canUndo()`; the `abUndo` vector's internals twice, via
  `UndoStacks::operator=` (`src/PluginProcessor.h:249`) against the reader's iteration; and the
