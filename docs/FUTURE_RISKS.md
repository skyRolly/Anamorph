...
- **Likelihood (evidence-based):** **Low.** It requires the HOST to write cross-parameter from
  ...
  `parameterGestureChanged` (`src/PluginProcessor.cpp:1321-1498`) touches two ints — the last
...
- **Risk:** `AnamorphAudioProcessor::parameterGestureChanged` counts open gestures and sets
  `pendingGestureCommit` when the count returns to zero (`src/PluginProcessor.cpp:1321-1497`), and
...
- **Evidence [Verified]:** `src/PluginProcessor.cpp:1321-1497` (the counter), `:827-834`
...
- **Risk (as recorded, now closed):** `getStateInformation`/`setStateInformation` mutate non-atomic message-thread-read
  ...
  `presets.setMeta`, `syncCommitted` (src/PluginProcessor.cpp:2896-2995 read
...
- **Round 21 (2026-09-02, ER-STATE-23 re-raised): re-measured on the current tree, same four
  ...
  (`setStateInformation`, `src/PluginProcessor.cpp:2896`) and with the same wording plus one added
  ...
  `src/PluginProcessor.cpp:2432`, against `canUndo()`; the `abUndo` vector's internals twice, via
