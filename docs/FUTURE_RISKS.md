...
- **Risk (as recorded, now closed):** `getStateInformation`/`setStateInformation` mutate non-atomic message-thread-read
  ...
  `presets.setMeta`/`adoptRestoredState`, `syncCommitted` (src/PluginProcessor.cpp:1239-1500 read
...
- **Round 21 (2026-09-02, ER-STATE-23 re-raised): re-measured on the current tree, same four
  ...
  (`setStateInformation`, `src/PluginProcessor.cpp:1445`) and with the same wording plus one added
  ...
  `src/PluginProcessor.cpp:1090`, against `canUndo()`; the `abUndo` vector's internals twice, via
