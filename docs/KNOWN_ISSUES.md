...
- **Evidence [Verified]:** src/dsp/AnamorphEngine.cpp:1179 (`mbStructuralChange` includes
...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  the only focus calls are src/PluginEditor.cpp:2362 (the on-open call) / src/PluginEditor.cpp:2406-2414 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
>   kept again once it has caught up, 2–4 s after a large change. So does a **new sample rate**, by
