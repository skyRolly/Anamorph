...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  passes up to four times (src/PluginEditor.cpp:2267-2275; declared src/PluginEditor.h:470). This shipped in
  ...
  the only focus calls are src/PluginEditor.cpp:2223 (the on-open call) / src/PluginEditor.cpp:2267-2275 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
- **Evidence [Verified (code path) / Unverified (the macOS-side attribution)]:**
  src/PluginEditor.cpp:381-391 (the field), src/PluginEditor.cpp:2211-2275 (show + focus);
