...
- **Mechanism:** on platforms **without per-pixel window alpha** (Linux/X11 with no compositor),
  ...
  the popup menu, which is kept square for exactly this reason (src/gui/LookAndFeel.cpp:623-626).
...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  passes up to four times (src/PluginEditor.cpp:2302-2310; declared src/PluginEditor.h:478). This shipped in
  ...
  the only focus calls are src/PluginEditor.cpp:2258 (the on-open call) / src/PluginEditor.cpp:2302-2310 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
- **Evidence [Verified (code path) / Unverified (the macOS-side attribution)]:**
  src/PluginEditor.cpp:381-391 (the field), src/PluginEditor.cpp:2240-2310 (show + focus);
...
- **Evidence [Verified]:** src/PluginProcessor.cpp:34-35, :117-134; pinned JUCE
