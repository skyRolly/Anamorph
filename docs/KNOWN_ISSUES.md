...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  passes up to four times (src/PluginEditor.cpp:2142-2150; declared src/PluginEditor.h:258). This shipped in
  ...
  the only focus calls are src/PluginEditor.cpp:2111 (the on-open call) / src/PluginEditor.cpp:2142-2150 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
- **Evidence [Verified (code path) / Unverified (the macOS-side attribution)]:**
  src/PluginEditor.cpp:375-385 (the field), src/PluginEditor.cpp:2100-2150 (show + focus);
