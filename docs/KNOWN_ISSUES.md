...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  passes up to four times (src/PluginEditor.cpp:2070-2078; declared src/PluginEditor.h:185). This shipped in
  ...
  the only focus calls are src/PluginEditor.cpp:2039 (the on-open call) / src/PluginEditor.cpp:2070-2078 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
- **Evidence [Verified (code path) / Unverified (the macOS-side attribution)]:**
  src/PluginEditor.cpp:363-373 (the field), src/PluginEditor.cpp:2028-2078 (show + focus);
