...
- **Current evidence [Partially Verified]:** The plugin already carries a **one-time** focus
  ...
  passes up to four times (src/PluginEditor.cpp:2139-2147; declared src/PluginEditor.h:257). This shipped in
  ...
  the only focus calls are src/PluginEditor.cpp:2108 (the on-open call) / src/PluginEditor.cpp:2139-2147 (`focusSaveNameField` itself) and the unrelated SpectrumImager freq editor).
...
- **Evidence [Verified (code path) / Unverified (the macOS-side attribution)]:**
  src/PluginEditor.cpp:375-385 (the field), src/PluginEditor.cpp:2097-2147 (show + focus);
