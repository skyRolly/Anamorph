...
`src/gui/LookAndFeel.cpp:402` and `src/dsp/VelvetNoise.cpp:30`, each an `int` widened inside a
...
`` src/PluginProcessor.cpp:271-281 (`updateLatency`) `` — and the checker now reads that gloss and
