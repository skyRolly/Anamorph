...
| Wrong/old JUCE behaviour | Stale fetched JUCE | Confirm the pinned commit `e18f7f5…` = JUCE 9.0.1 (CMakeLists.txt:65-67); a JUCE bump is a Build System change (ARCHITECTURE_REVIEW_GATE, ADR-0022 / ADR-0026). |
...
| Linker errors mixing JUCE modules | DSP compiled as a STATIC lib | The DSP core is an **INTERFACE** lib by design (CMakeLists.txt:335-346) — keep it INTERFACE; do not pre-compile JUCE modules into a static lib. |
