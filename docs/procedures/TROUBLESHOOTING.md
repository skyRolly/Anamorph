...
| Wrong/old JUCE behaviour | Stale fetched JUCE | Confirm the pinned commit `7278278…` — CMakeLists.txt:71 (`7278278`) — = JUCE 9.0.2 — CMakeLists.txt:70-72 (`9.0.2`); a JUCE bump is a Build System change (ARCHITECTURE_REVIEW_GATE, ADR-0022 / ADR-0026 / ADR-0054). |
| Configure says `fetching JUCE 9.0.2 (<old rev>)` | `ANAMORPH_JUCE_TAG` is a CACHE variable — an existing `build/` keeps the OLD pin after a pull | Delete `build/` (or `cmake -B build -UANAMORPH_JUCE_TAG -UANAMORPH_JUCE_VERSION`) so the new pin takes effect; the configure banner prints version + rev precisely so a mismatch is visible. The pin block, comment included, is CMakeLists.txt:67-72 (`9.0.2`). |
