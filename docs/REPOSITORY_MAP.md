...
│                           graduate to ADRs; worklogs are the raw evidence trail). Some
│                           worklogs also have a rendered .html companion beside them
│                           (the three performance/PERF_AUDIT_*_REPORT.html pages, and
│                           engineering-review/ENGINEERING_REVIEW_REPORT.html — the
│                           standing review programme's live dashboard, updated and
│                           re-committed each round). Every companion is a VIEW of the
│                           .md beside it, never a second source: the worklog is what a
│                           later round reconciles against. (The map said "one worklog
│                           has a companion" for two rounds after there were three —
...
| Path | Responsibility |
|---|---|
| `tests/dsp_tests.cpp` | 53 headless DSP acceptance tests + 1 A/B state-restoration clamp guard (`check(cond, "...")` harness; `main` runs all). Also carries four opt-in probes that assert nothing and print: `--match-inject-probe`, `--os-latency-probe`, `--forced-swap-probe` and `--os-off-probe`. |
...
| `tests/tsan_canary.cpp` | Liveness proof for the `tsan` job (D-2 / ADR-0036): two threads write one plain `int` with no synchronisation, so the job can assert that ThreadSanitizer is instrumenting before its silence over the four cross-thread probes is trusted. Compiled directly by the workflow step, like its RTSan sibling (no CMake target). |
...
| `tests/state_tests.cpp` | 40 headless state-compatibility tests (schema shape, parameter-registry snapshot, raw-exact round-trip, 3 legacy migration fixtures, corrupt-state robustness, preset round-trip, A/B + view-param preservation, factory/user preset identity under a shared name, factory-id integrity, indicator identity across a session reload, wrapper audio path through the real `processBlock`, first activation honouring a restored session, and the **cross-version field capture** — a session written by a rebuilt v0.9.5 binary reproduced exactly, which is what discharges the compatibility checklist's "Session reload verified" box, and the legacy A/B reset — a restore carrying no A/B data must not leave the previous project's slots loaded — and the restore/latency integrity guards: a root claiming to be ours but carrying no `ANAMORPH` sound child must adopt no metadata, the D-1 request/timer machinery must leave the host holding what the settled state predicts, and a malformed Setting in a pre-0.8.4 session must resolve to a valid setting rather than an undefined integer conversion, and a `prepareToPlay` on a thread that is not the message thread must deliver no latency report from that thread and still be served the prepared value by the timer, and a restore with no A/B data must not carry the previous session's per-slot Level-Match gains into the first switch, and a malformed scope-persistence value must not reach the vectorscope non-finite, and a present-but-invalid modern Setting must be repaired on restore and the repaired value persisted, and — D-2 / ADR-0036 — the program-state ownership contract under off-message-thread host restores and saves: A/B coherence, restore cycles with the audio thread running, preset transitions under concurrent saves, restore with re-prepare, and the undo history a host restore only clears) plus the four opt-in ThreadSanitizer probes the `tsan` job runs — own console target `AnamorphStateTests` compiling the plugin sources. |
| `tests/fixtures/` | Compatibility fixtures: `parameter_registry.snapshot` (re-frozen only via `AnamorphStateTests --write-snapshot` for INTENTIONAL parameter changes) + 3 frozen legacy session XMLs (v0.2 / pre-0.6.4 / pre-0.8.4) + `field_capture_v0_9_5.session` and its `.manifest` — the one fixture that is **not** a reconstruction: a real `getStateInformation` blob WRITTEN by a v0.9.5 binary rebuilt from the tree at `2c5e760^`, with the manifest recording what that binary believed the state was. The three legacy XMLs can only contain what current code thinks an old format held; this one cannot. |
| `scripts/setup-linux.sh` | Linux build dependencies, in two profiles. `full` (the default, and what a developer runs) = the core set + the host toolchain, curl/unzip for the pluginval fetch, xvfb and lld for the Clang/LTO link. `headless` = only what compiling and linking the JUCE targets needs — used by the GCC compatibility job, which runs inside a toolchain container and builds no GUI. Both name `python3` explicitly, since every checker in `scripts/` is Python and a container may carry an interpreter only transitively. |
| `scripts/setup-llvm-apt.sh` | Installs ONE Clang major (compiler + `lld` + `libclang-rt`) from **apt.llvm.org** for the **five** jobs that consume it — `merge-check`, `linux`, `sanitizers`, `realtime` and `fuzz`, of which `linux` builds the SHIPPED artifact (ADR-0030) — since Ubuntu's archives stop at `clang-20` for noble while the pin is upstream stable (ADR-0028). Takes the major as an argument so `ANAMORPH_CLANG_VERSION` stays the single authority; reads the suite codename from `/etc/os-release`; verifies the apt.llvm.org signing key by fingerprint. **Verification is RELEASE IDENTITY, not the major** (ADR-0033): the script carries the upstream release version and its `llvmorg-<version>` tag commit per major, refuses a major it has no identity for (**exit 2**), and reads the build commit from four sources — `clang --version` and the installed version of each of the three packages. Every source that carries a commit must agree, **at least one must carry one**, and it must be the tag's; otherwise the install **fails closed** (exit 1), because a version string alone cannot separate a release from a branch build wearing the same number. `--self-test` drives that decision function with recorded strings (no apt, no network) and runs in `source-lint` and `preflight.sh`. |
...
| `scripts/ubsan-ignorelist.txt` | Clang instrumentation ignorelist for the `sanitizers` job, holding exactly one rule: `implicit-conversion` is not instrumented under `*juce-src/*`. Vendored third-party code inside JUCE (HarfBuzz's `num_glyphs = -1` sentinel, reached once the editor-lifetime test made the constructor shape text) narrows by idiom, which is neither a defect nor this project's code. The section header scopes it to that ONE sub-check — ASan, the rest of `undefined`, `vptr` and the four other groups still cover the vendored tree — and the glob scopes it to the dependency checkout, which no first-party path matches. |
| `scripts/check-docs.py` | Structural Markdown lint over the whole document set (table integrity, relative links, blockquote lazy continuation, unclosed fences; for `CHANGELOG.md` also the entry-heading grammar and newest-first order, the Keep a Changelog category names/order/once-each rule, the version link definitions, and the entry-boundary rule). `--self-test` first. |
| `scripts/check-realtime.py` | Static realtime lint (`source-lint`): scans the bodies of the functions `REALTIME_AUDIO_POLICY` binds — `processBlock` (wrapper included, so the scan root is `src`, not `src/dsp`), `process`, every module's `reset`/`softReset`, and `setParameters`/`toEngine` (ER-RT-02: `processBlock` calls both every block) — **plus every callee defined in the same file, transitively**, for its forbidden list (allocation, container growth, locks, threading, IO). **Function-scoped on purpose** — `prepare()` is required to allocate, so a file-wide token scan would flag the eight legitimate `setSize` calls in `AnamorphEngine.cpp`; comments and string literals are blanked before matching. That closure is **same-file**: a callee defined in another translation unit is covered only if its own name is a seed, which is the enforcement boundary `REALTIME_AUDIO_POLICY.md` and `REALTIME_SAFETY_AUDIT.md` state and the periodic census re-measures. The third realtime tier (ADR-0029): the only one that reads code the suite never executes. `--self-test` proves it fires and stays quiet — including over the two lexical constructs that would otherwise blank a line and swallow a real violation (an unbalanced quote inside a raw string; an encoded character literal such as `L'a'`, whose opening quote has the same alphanumeric-both-sides shape as a digit separator), and over a definition whose opening brace sits far from its signature — the brace search is unbounded, because a bounded one dropped such a definition from the scanned set with no diagnostic. |
...
| `scripts/check-portability.py` | Rejects explicit template arguments on `juce::jmin/jmax/snapToZero` (instantiates `dsp::SIMDRegister<T>` — compiles on Linux, fails on macOS); also checks the Linux installer/uninstaller scratch-name sets agree. Two different proofs: `--self-test` (in `source-lint`, beside the lint) proves the **checker** still fires and still stays quiet; `--compile-canary` (in `linux`, where JUCE is checked out) proves the **hazard** still exists in the pinned JUCE. |
| `scripts/check-citations.py` | Keeps `file.cpp:NNN` evidence anchors in `docs/` pointing at the text they named at a base revision, and — for the eight architecture documents in `GLOSS_CHECKED_DOCS` — that each anchor whose citation names a symbol beside it actually contains that symbol, which is the one question the base comparison structurally cannot ask. Governs `src/`, `tests/`, the four `scripts/`, **`CMakeLists.txt` and `.github/workflows/build.yml`** — the last two added after 96 anchors into them drifted while the gate reported clean; `packaging/*/INSTALL.txt` and `NOTICE` remain outside it. `--check` reports drift, `--fix` re-anchors; deliberate re-aims are declared in `DELIBERATE_REAIMS` — keyed on the **transition** `(document, base anchor, current anchor)`, so a declaration authorises one movement and expires the moment either end differs, rather than exempting its anchor for ever — and a line whose CONTENT changes on its own schedule -- `CMakeLists.txt:14`, the `project(... VERSION ...)` line -- is declared in **`VERSIONED_LINES`**, which replaces the base comparison for that one line with a permanent token check (`verify_versioned_lines()`, a hard failure in every mode) and applies only while the anchor has not moved. Added for 0.9.5, the first version bump after this file came under the gate, which the gate blocked. `--self-test` proves the ownership test, the line map and the span rewriter are live — this is the only lint that **writes** to the documents, so a defect corrupts rather than merely misses. `--fix` also **reports the `DELIBERATE_REAIMS` declarations its own rewrite invalidates**, naming the replacement spelling: the self-test catches a dead declaration, but in CI minutes later and without knowing what killed it. A misaimed declaration is a hard failure in `--check` (the mode CI runs) but only a warning in `--fix`, which then repairs everything else and still exits non-zero: declared anchors drift for ordinary reasons — two are single lines in `build.yml` — and stopping the whole run over one blocked the repair of every unrelated citation too (measured: 0 re-anchored vs 31 across 16 documents). The document that **owns** the misaimed declaration is withheld from the rewrite, because re-deriving one anchor while its excused neighbour stays put is how a sentence ends up naming one line twice. |
| `scripts/check-clang-warnings.py` | The first-party warning gate for the `linux` job (the Clang release build): classifies each Clang diagnostic **structurally** by resolved path (`src/`, `tests/`, never through `_deps`). `--self-test` proves the classifier is live. |
...
| `.github/actions/setup-linux-build/` | Composite action carrying the Linux setup the Linux jobs share: `setup-linux.sh` at the requested `dependency-profile` (`full` by default, `headless` for the containerised GCC job), optionally the pinned Clang (`clang-version`) and extra apt packages (`extra-packages`, both fail-closed), then the ccache install behind its non-fatal fallback. **The ccache fallback is a policy, and this is its one copy** — it used to be written out seven times, six of them byte-identical. The per-job cache **lineage** deliberately stays in `build.yml`: that part genuinely differs per job. |
| `.github/workflows/build.yml` | 3-OS build + DSP **and state** self-tests + pluginval (both modes ×3, **blocking on all three platforms**, VST3 everywhere and **AU on macOS**; strictness held once in `env.ANAMORPH_PLUGINVAL_STRICTNESS`); plus `macos-intel` on `macos-15-intel`, which runs the same suites and the same VST3+AU gate against a thin `x86_64` build on **native Intel** hardware and ships nothing (the `macos` job's Rosetta step executes the *shipped* slice but on arm64 hardware, so the two are complements); plus nine non-packaging jobs with no `needs:` in either direction — `merge-check` (the same build from `refs/pull/N/merge`; its `if:` is the exact complement of every other job's, so it runs only on the same-repo PR event and is the only job that runs there), `docs`, `source-lint`, `sanitizers` (ASan+UBSan with **LeakSanitizer now a gate**, then valgrind memcheck), `linux-lto-tests` (**GCC compatibility build — floating `gcc:16` container, major pinned, patch not**: both suites built and run with `-flto`, the only place a behavioural assertion executes LTO codegen, plus the GCC-only warning gate and the liveness builds of both committed instruments — the benchmark build+smoke and `AnamorphDspDump --self-check`; `linux` compiles the same two TUs `-fsyntax-only` under the pinned Clang, the compiler this container does not have), `realtime` (the DSP suite under **RealtimeSanitizer** behind a liveness canary, plus the leaf-layer `-Werror=function-effects` check — the first mechanical detectors for `REALTIME_AUDIO_POLICY`; ADR-0029), `fuzz` (`setStateInformation` under libFuzzer, bounded, corpus-seeded), `macos-crossslice` (the twin dump for both universal slices, arm64 + x86_64-under-Rosetta; **reporting-only** — job-level `continue-on-error`, ADR-0031 Class-B scoping) and `windows-avx2-ab` (baseline vs `/arch:AVX2` twin-dump diff; **blocking** — the ADR-0032 per-push Class-A assertion). Stages the per-platform packages — flat `Anamorph-<OS>` artifacts (loose files; release.yml archives the release zip from the same tree), Windows/macOS installers, `-debug` symbols. One run per ref (`concurrency`, tags exempt from cancellation); same-repo PR events skipped as duplicates of the branch push. Every job carries an explicit `timeout-minutes`, and every Ninja job compiles through **ccache** restored from `actions/cache` (Windows excepted — `/Zi` is required for the shipped PDB). Also callable (`workflow_call`) by release.yml. |
...
│                    RELEASE_COMPATIBILITY_CHECKLIST, LEVEL5_AUDITION, TROUBLESHOOTING)
...
Evidence [Verified]: file tree from the repository; CMakeLists.txt:114-384 (hardening interface) + :326 (`AnamorphDSP`) / :352 (`juce_add_plugin`); src/ listing.
