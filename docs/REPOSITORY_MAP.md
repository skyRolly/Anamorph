...
| `tests/AllocationGuard.h` | The audio-path allocation guard (Test 38, ADR-0029): replaceable `operator new`/`delete` plus glibc malloc-family interposition, armed only around `process()`. The tier that reaches **MSVC**, where RTSan does not run. Self-checks its counters before reporting a zero; compiled out for the valgrind build (`ANAMORPH_NO_ALLOC_GUARD`) and its malloc half under ASan, each disclosed with a `::warning::`. |
...
| `scripts/preflight.sh` | Local pre-push aggregate (2026-08-18): the four lints with their `--self-test`s, the citation gate against **both** bases that can disagree (`origin/main` and the branch merge base), then `run-tests.sh` when a built tree exists — skipped **with a note**, never silently. States its own limit: the full Clang warning gate needs a clang build log, so only that lint's self-test runs locally. |
