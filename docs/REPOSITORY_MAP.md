...
| Path | Responsibility |
|---|---|
| `tests/dsp_tests.cpp` | 52 headless DSP acceptance tests + 1 A/B state-restoration clamp guard (`check(cond, "...")` harness; `main` runs all). Also carries two opt-in probes that assert nothing and print: `--match-inject-probe`, `--os-latency-probe` and `--forced-swap-probe`. |
