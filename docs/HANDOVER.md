...
| Field | Value |
|---|---|
| **Current Version** | 0.8.10 (`CMakeLists.txt:14`; **released** with PR #59 — the undo/redo forced-duck dry-fill crossfade + rapid-swap robustness, the multiband flat-recombination crossover fix, the adaptive `FrameClock` display-rate GUI refresh, plus three pre-merge correctness rounds: the split-movement fix (final design after three measured iterations + a follower refinement, all recorded in **ADR-0015**: a ~1.25 oct/s inaudibility-capped cutoff glide, a discrete-jump bank crossfade, and release consolidation for bounded post-drag convergence in `MultibandWidth`/`SoloMonitor`; the interim chained-crossfade and one-pole designs were measured audible on pure sines and replaced; the H3 gain-domain proposal failed hostile review on width purity — trade documented as KI-012), the forced-duck dry-fill output-gain latch (Test 30, no spike at extreme Output Gain), the Band Solo alt-click exclusive-solo redesign, the Option-click reset undo fix, and the Apple-Silicon-native tooltip corner fix (KI-011, hardware re-test pending) — all folded into the `[0.8.10]` CHANGELOG entry). Prior: 0.8.9 (PR #58, Wave-2 performance H3/H4/H5/H6/H11/H15/ALG-4 + a lifecycle fix). |
...
| **Test Status** | 29 DSP self-tests **+ an A/B state-restoration clamp guard** (106 individual assertions), and pluginval strictness 10 on the Linux gate in **both modes** (deterministic + `--randomise` ×3, blocking). The JUCE 8.0.14 bump was green on CI (`41acaa7`); the randomise gate + clamp guard were added afterwards; the newest are `testMultibandSplitDragNoPitchShift` (Test 29: worst 100 ms pitch chunk < 5 cents through drags, the ~1.25 oct/s ease and an unbroken tone-crossing crawl — rejected designs measure 24–50 cents and fail; max spur < −31 dBc — the chained bank fades measure −28.5 and fail; discrete jumps land < 200 ms; a released 6-oct flick lands < 0.5 s via the ADR-0015 release consolidation — the crawl-only follower fails at full level) and `testDryFillRespectsOutputGain` (Test 30: no dry-fill spike at Output Gain −24 dB — the unscaled fill measures 15.8× and fails). `docs/procedures/TESTING.md`. |
