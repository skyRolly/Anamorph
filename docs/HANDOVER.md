...
| Field | Value |
|---|---|
| **Current Version** | 0.8.10 (`CMakeLists.txt:14`; **released** with PR #59 — the undo/redo forced-duck dry-fill crossfade + rapid-swap robustness, the multiband flat-recombination crossover fix, the adaptive `FrameClock` display-rate GUI refresh, plus two pre-merge correctness rounds: the split-drag transition fix (final design: bounded-time one-pole cutoff glide + a single bank crossfade for multi-octave jumps in `MultibandWidth`/`SoloMonitor` — the interim chained-crossfade version sprayed audible sidebands around a pure tone and was replaced), the Band Solo alt-click exclusive-solo redesign, the Option-click reset undo fix, and the Apple-Silicon-native tooltip corner fix (KI-011, hardware re-test pending) — all folded into the `[0.8.10]` CHANGELOG entry). Prior: 0.8.9 (PR #58, Wave-2 performance H3/H4/H5/H6/H11/H15/ALG-4 + a lifecycle fix). |
...
| **Build Status** | Green at v0.8.10 (undo/redo + multiband + FrameClock + correctness round PR #59; local build + 97-check DSP self-tests green; pluginval runs on the CI Linux gate — the local sandbox cannot fetch the pluginval release under the egress policy, the same constraint recorded for the JUCE bump). Earlier: v0.8.9 (PRs #55/#56/#57/#58); the **JUCE dependency was bumped 8.0.8 → 8.0.14** (`41acaa7`, ADR-0012) and **verified green by CI** — build + DSP self-tests + `pluginval PASSED at strictness 10` on the Linux gate. The v0.8.10 undo/redo + FrameClock work is behaviour-preserving (single forced swaps byte-identical; GUI message-thread only); the multiband flat-recombination fix intentionally changes the multiband audio (removes the crossover dip) with reported latency unchanged. Build = CMake + JUCE 8.0.14, VST3 [+AU macOS] [+Standalone]. |
| **Test Status** | 28 DSP self-tests **+ an A/B state-restoration clamp guard** (97 individual assertions), and pluginval strictness 10 on the Linux gate in **both modes** (deterministic + `--randomise` ×3, blocking). The JUCE 8.0.14 bump was green on CI (`41acaa7`); the randomise gate + clamp guard were added afterwards; the newest is `testMultibandSplitDragNoPitchShift` (Test 29: post-drag pitch < 5 cents on the multiband and solo-monitor paths — the rate-capped glide measures ~24 cents and fails — plus max spur < −31 dBc around a pure tone while the split crosses it — the interim chained bank fades measure −28.5 dBc and fail), after `testRapidForcedSwapDryFill` (Test 27) and `testMultibandFlatRecombination` (Test 28). `docs/procedures/TESTING.md`. |
