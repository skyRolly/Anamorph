...
| **Test Status** | 30 DSP self-tests **+ an A/B state-restoration clamp guard** (112 individual assertions), and pluginval strictness 10 on the Linux gate in **both modes** (deterministic + `--randomise` ×3, blocking). Newest: `testForcedSwapDuringOrdinaryFadeOut` (Test 31: a forced swap landing mid-ordinary-fade-out keeps forced semantics — the pre-fix engine dropped the request and replays a 0.494-peak stale Haas tail, failing). The JUCE 8.0.14 bump was green on CI (`41acaa7`); the randomise gate + clamp guard were added afterwards; the newest are `testMultibandSplitDragNoPitchShift` (Test 29: worst 100 ms pitch chunk < 18 cents — the accepted controlled-FM bound of the ~4 oct/s cap, ADR-0015 final — through drags, the whole catch-up and an unbroken tone-crossing crawl (the shipped cap measures ~14; the uncapped ~8 oct/s glide measures ~28 and the one-pole ~50, both fail); max spur < −31 dBc — the chained bank fades measure −28.5 and fail; discrete jumps land < 200 ms; a released 6-oct flick lands by plain gliding ~1.5 s in — the rejected 1.25 oct/s follower fails at full level) and `testDryFillRespectsOutputGain` (Test 30: no dry-fill spike at Output Gain −24 dB — the unscaled fill measures 15.8× and fails). `docs/procedures/TESTING.md`. |
