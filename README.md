...
## Project status
- **Version 0.9.9** (pre-1.0), in the **internal-testing phase** — builds go to testers for
...
- Validation gate: **53 DSP self-tests** + the **63-test state-compatibility suite** + **pluginval strictness 10** (both modes ×3, blocking on all three CI platforms).
...
## Requirements
- **Runtime, x86-64:** an Intel **Haswell** (2013) / AMD **Excavator** (2015) CPU or newer for
  **every** x86-64 binary — the Linux build and the macOS `x86_64` slice, compiled
  `-march=haswell` (ADR-0031), and the **Windows** build, compiled `/arch:AVX2` (ADR-0032).
